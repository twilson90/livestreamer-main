var app = (function (exports, core_js) {
    'use strict';

    const { UI, add_class, remove_class, toggle_attribute, set_inner_html, set_children, set_attribute, set_select_options, set_text, set_value, set_style_property, update_style_properties, remove_style_property, toggle_class } = core_js.dom_utils;
    const { Observer } = core_js.utils;

    core_js.Sortable.mount(new core_js.MultiDrag());

    // if (window.videojs) window.videojs.options.autoplay = true;
    // export const WS_MIN_WAIT = 1000;
    const WS_MIN_WAIT = 0;
    const MIN_VIDEO_BUFFER_TIME = 1000; // 1 second

    const IMAGE_DURATION = 0.040;
    const CROP_LIMIT = 0.4;
    const IS_ELECTRON = /electron/i.test(navigator.userAgent);
    // console.log("IS_ELECTRON:", IS_ELECTRON);

    const logs_max_length = 512;
    const ZERO_DURATION = 60;
    const VOLUME_STEP = 5;
    const MAX_CLIP_SEGMENTS = 128;
    const ELFINDER_USE_FILE_PROTOCOL = false;
    const EMPTY_OBJECT = Object.freeze({});
    const EMPTY_ARRAY = Object.freeze([]);
    const ALL_XHRS = new Set();

    window.onbeforeunload = (e)=>{
        if (ALL_XHRS.size) return `Uploads are in progress, leaving will abort the uploads.`;
        // return "";
    };
    const YES_OR_NO = [[false,"No"], [true,"Yes"]];

    const ignore_logging_session_$ = new Set([
        "time",
        "player/time-pos",
    ...
    const UPLOAD_STATUS = { STARTED:1, FINISHED:2, CANCELED:3, ERROR:4 };
    const PLAYLIST_VIEW = { LIST: "list", TIMELINE: "timeline" };
    const PLAYLIST_MODE = { NORMAL: 0, MERGED: 1, DUAL_TRACK: 2 };
    ...
    function moving_average(points, windowSize) {
...

    {
        let media_type = function(type) {
            var v = this.value;
            if (!v) return true;
            var mi = exports.app.$.media_info[v];
    ...
    var default_file_manager_options = {
        files: false,
        folders: false,
        multiple: false,
    };
    ...
    var graph_colors = ['#e6194B', '#3cb44b', '#4363d8', '#f58231', '#911eb4', '#42d4f4', '#ffe119', '#f032e6', '#bfef45', '#fabed4', '#469990', '#dcbeff', '#9A6324', '#800000', '#aaffc3', '#808000', '#ffd8b1', '#000075', '#a9a9a9', '#ffffff', '#000000'];
    ...
    (()=>{
        for (var k in item_colors) {
            ...
            item_colors[k] = new core_js.utils.Color(item_colors[k]).rgb_mix("#fff",0.5).to_rgb_hex();
        }
    })();
    ...
    UploadFileChunk.create = function(blob, path=undefined) {
        ...
        ufc._blob = blob;
        ufc.id = blob.id || core_js.dom_utils.uuid4();
    ...
    class UploadQueue {
        ...
        add(files, opts) {
            ...
            for (var c of chunks) {
                if (opts.dir) c.path = core_js.utils.join_paths(opts.dir, c.path);
            ...
            while (!done) {
                ...
                let response = await new Promise((resolve) => {
                    ...
                    xhr.addEventListener("loadend", (e) => {
                        resolve(xhr.readyState == 4 && core_js.utils.try(()=>JSON.parse(xhr.responseText)));
                    });
                ...
                if (xhr.canceled || core_js.utils.try(()=>response.uploads[c.id].status === UploadStatus.CANCELED)) {
                ...
                if (!done) {
                    ...
                    await core_js.utils.timeout(5000);
                }
            }
            this.next_chunk();
        }
    ...
    class FileDrop extends core_js.utils.EventEmitter{
    ...
    function remove_empty_objects_from_tree(obj) {
        var deletes = 0;
    ...
    function get_scrollbar_width(el) {
        return [el.offsetWidth - el.clientWidth, el.offsetHeight - el.clientHeight];
    }

    function pretty_uri_basename(uri) {
        if (uri.match(/^https?:/)) {
            return uri;
        } else {
            var name = core_js.utils.basename(uri);
            try { name = decodeURI(name); } catch {}
            return name;
        }
    }
    function rect_clamp_point(rect, pt) {
        return {x:core_js.utils.clamp(pt.x, rect.x, rect.x+rect.width), y:core_js.utils.clamp(pt.y, rect.y, rect.y+rect.height)};
    }
    ...
    function get_clip_segments(start, end, duration, offset=0) {
        ...
        var t = core_js.utils.loop(start + offset, start, end);
    ...
    async function read_file(file, encoding="utf-8") {
        if (file instanceof File) {
            return core_js.dom_utils.read_file(file, {encoding})
        } else if (IS_ELECTRON) {
    ...
    async function open_file_dialog(options) {
        if (IS_ELECTRON)  {
            ...
            return paths.map(p=>({path:p, name:core_js.utils.basename(p)}));
        } else {
            var dialog_opts = {};
            if (options.filter) dialog_opts.accept = options.filter.join(", ");
            if (options.multiple) dialog_opts.multiple = !!options.multiple;
            return await core_js.dom_utils.open_file_dialog(dialog_opts);
        }
    }
    async function save_local_file(filename, text) {
        if (IS_ELECTRON)  {
            ...
            core_js.dom_utils.download(filename, text);
            return true;
        }
    }


    function get_video_size(w,h,interlaced) {
        var ratio = w / h;
    ...
    function create_background_properties(options, is_session) {
        ...
        var get_file_duration = ()=>exports.app.$.media_info[background_file.value] ? exports.app.$.media_info[background_file.value].duration : 0;
    ...
    class TicksBar {
        ...
        constructor(elem, opts) {
            ...
            this.start = 0;
            this.end = 0;
            this.elem = elem || core_js.$(`<div></div>`)[0];
            add_class(this.elem, "ticks-bar");
            this.elem.dataset.placement = opts.placement;
            if (!opts.hover_elem) opts.hover_elem = elem;

            this.ticks_elem = core_js.$(`<div class="ticks"></div>`)[0];
            var cursor_elem = core_js.$(`<div class="cursor"></div>`)[0];
            var seek_time = core_js.$(`<div class="seek-time"></div>`)[0];
            ...
            var update_seek_time = (e)=>{
                ...
                var html = `<div>${core_js.utils.seconds_to_timespan_str(data.time, exports.app.user_time_format)}</div>`;
                set_inner_html(seek_time, `<div>${opts.modifier(html, data.time)}</div>`);
            };

            this.hover_listener = new core_js.dom_utils.TouchListener(opts.hover_elem, {
                mode: "hover",
                start: (e)=>{
                    // console.log("in")
        ...
        parse_event(e) {
            var rect = new core_js.utils.Rectangle(this.elem.getBoundingClientRect());
        ...
        update(start, end) {
            ...
            if (duration != 0 && duration < TicksBar.max_tick_time) {
                ...
                var max_i = core_js.utils.clamp(min_i + TicksBar.tick_heights.length-1, 0, TicksBar.tick_times.length-1);
                // var tis = [];
                for (var t = start; t <= end; t += min_divisor) {
                    ...
                    if (this.opts.show_numbers) {
                        if ((ti >= min_i+2) || (num_ticks < TicksBar.max_ticks*0.25 && ti >= min_i+1)) {
                            text = `<span>${core_js.utils.ms_to_shorthand_str(tr*1000, core_js.utils.log(Math.ceil(1/TicksBar.tick_times[ti]), 10)).replace(/\s+/g,"")}</span>`;
                        }
                    }
    ...
    async function open_file_manager(options) {
        options = Object.assign({
            "new_window" : exports.app.settings.get("open_file_manager_in_new_window")
        }, default_file_manager_options, options);
        if (!options.standalone && options.id === undefined) options.id = core_js.dom_utils.uuidb64();
        ...
        if (IS_ELECTRON) {
            ...
            if (options.start) electron_options.defaultPath = core_js.utils.try_file_uri_to_path(options.start); // utils.dirname(options.start[0]);
            ...
            if (options.filter) {
                ...
                for (var f of options.filter) {
                    var m;
                    if (m = f.match(/^(.+?)\/\*$/)) {
                        electron_options.filters.push({ name: core_js.utils.capitalize(m[1])+"s", extensions: mime_ext_map[m[1]] || ["*"]});
                    } else if (m = f.match(/^\.(\w+)$/)) {
                        custom_ext.push(m[1]);
                    }
                }
                if (custom_ext.length) {
                    custom_ext = core_js.utils.array_unique(custom_ext);
        ...
        } else if (exports.app.$.processes["file-manager"]) {
            /** @type {Window} */
            var win;
            var win_id = options.hidden_id || options.id;
            var use_window = options.new_window;
            var on_message;
            var messenger = new core_js.dom_utils.WindowCommunicator();
            return new Promise((resolve,reject)=>{
                ...
                if (options.start) {
                    ...
                    elfinder_options.startPathHash = exports.app.filename_to_elfinder_hash(dir);
                    elfinder_options.selectHashes = [exports.app.filename_to_elfinder_hash(options.start)];
                }
                if (options.filter) {
                ...
                messenger.on("files", ({files,id})=>{
                    ...
                    var paths = files.map(f=>core_js.utils.try_file_uri_to_path(f.uri));
                    resolve(paths);
                });
                ...
                if (use_window) {
                    ...
                    exports.app.file_manager_menu.show(url);
                    win = exports.app.file_manager_menu.iframe.contentWindow;
                    exports.app.file_manager_menu.once("hide", ()=>{
                        resolve();
                    });
                }
            }).finally(()=>{
                ...
                else exports.app.file_manager_menu.hide();
            })
        } else {
    ...
    function create_context_menu(parent, elem, e) {
        var on_click;
        var t = core_js.dom_utils.tippy(parent, {
            appendTo: exports.app.elem,
            content: elem,
            onHide: (instance) =>{
                exports.app.root_elem.removeEventListener("click", on_click);
            },
        ...
        t.show();
        setTimeout(()=>{
            exports.app.root_elem.addEventListener("click", on_click = (e)=>{
                if (!core_js.dom_utils.closest(e.target, el=>el==t.popper)) t.hide();
            });
        },0);
        return t;
    }

    function create_menu(items, o={}) {
        var opts = {
            params: [],
            click: NOOP,
        };
        Object.assign(opts, o);
        var list = core_js.$(`<ul class="list-menu"></ul>`)[0];
        ...
        var process_item = (item, list)=>{
            ...
            if (is_separator) {
                elem = core_js.$(`<li class="separator"><hr></li>`)[0];
            } else if (Array.isArray(item)) {
                elem = core_js.$(`<li class="group"><ul></ul></li>`)[0];
                item.forEach(i=>process_item(i, elem.children[0]));
            } else {
                elem = core_js.$(`<li class="item"></li>`)[0];
                ...
                if (icon) elem.append(...core_js.$(`<span class="icon">${icon}</span>`));
                if (name) elem.append(...core_js.$(`<span class="name">${name}</span>`));
                if (shortcut) {
                    shortcut = shortcut.replace("ArrowUp","↑").replace("ArrowDown","↓").replace("ArrowLeft","←").replace("ArrowRight","→");
                    elem.append(...core_js.$(`<span class="shortcut"><span>${shortcut}</span></span>`));
                }
                if (disabled) {
                    elem.disabled = true;
    ...
    function fancybox_prompt(title, inputs, settings) {
        ...
        var props = inputs.map(input=>{
            ...
            if (input instanceof UI.Property) {
                prop = input;
            } else if (core_js.dom_utils.is_html(input)) {
    ...
    core_js.Sortable.mount(CancelSortPlugin);

    var Orientation = {
        VERTICAL: "vertical",
        HORIZONTAL: "horizontal"
    };
    class ResponsiveSortable extends core_js.Sortable {
        constructor(el, options) {
            ...
            if (this.options.multiDrag && !this.options.handle) {
                ...
                this._triggerDragStart = (evt, touch)=>{
                    ...
                    if (touch && !this.is_selected(item)) {
                        core_js.Sortable.utils.on(this.el.getRootNode(), 'touchmove', on_touch_move);
                        return;
                    }
                ...
                this._onDrop = (evt)=>{
                    core_js.Sortable.utils.off(this.el.getRootNode(), 'touchmove', on_touch_move);
            ...
            window.addEventListener("keydown", this._on_key_down = (e)=>{
                if (!core_js.dom_utils.has_focus(this.el, true)) return;
                ...
                if (next_index !== undefined) {
                    next_index = core_js.utils.clamp(next_index, 0, items.length-1);
            ...
            this.el.addEventListener("unchoose", this._on_unchoose = (e)=>{
                ...
                var selected_items = items.filter(i=>core_js.Sortable.utils.get(i).is_selected(i));
                for (var s of this.get_sortables_in_group()) {
                    if (s !== this) s.deselect_all();
                }
                var dest = core_js.Sortable.get(e.to);
        ...
        get_last_active(use_fallback=false) {
            ...
            var items = this.get_items();
            return items[core_js.utils.clamp(this.last_active_index || 0, 0, items.length-1)];
        }
        ...
        get_items(filter=null) {
            var items = Array.from(this.el.children).filter(e=>core_js.Sortable.utils.closest(e, this.options.draggable, this.el, false));
            if (filter) {
        ...
        select(item) {
            ...
            core_js.Sortable.utils.select(item);
        ...
        deselect(item) {
            item = this.get_item(item);
            if (!item || !this.is_selected(item)) return;
            core_js.Sortable.utils.deselect(item);
        ...
        get_item(e) {
            e = core_js.Sortable.utils.closest(e, this.options.draggable, this.el, false);
        ...
        destroy() {
            ...
            core_js.utils.array_remove(ResponsiveSortable.instances, this);
    ...
    core_js.Sortable.utils.get = function(e) {
        if (!e) return null;
        if (e instanceof core_js.Sortable) return e;
        var el = core_js.dom_utils.closest(e, c=>core_js.Sortable.get(c));
        return el ? core_js.Sortable.get(el) : null;
    };

    const NOOP = ()=>{};
    var get_or_call = (f, context, params)=>{ return typeof f === "function" ? f.apply(context, params) : f; };
    ...
    */
    ...
    class AccessControl {
        ...
        get users() {
            ...
            return core_js.utils.sort(users, v=>(v.username=="*")?0:1, v=>v.access=="owner"?0:1, v=>AccessControl.ACCESS_ORDER[v.access], v=>v.username.toLowerCase());
        }
        get owners() { return this.users.filter(d=>d.access==="owner"); }
        edit(username, data) {
            var user = this.data[username];
            if (data == null) {
                if (user && user.access === "owner" && username === exports.app.$._client.username && this.owners.length > 1) {
            ...
            if (this.owners.length == 0) {
                core_js.utils.clear(this.data);
                Object.assign(this.data, { "*": {"access":"allow"} });
            }
            return true;
        }
        get self_can_edit() { return this.owners.length == 0 || this.self_is_owner_or_admin; }
        get self_is_owner() { return this.has_ownership(exports.app.$._client.username); }
        get self_is_owner_or_admin() { return this.self_is_owner || exports.app.$._client.is_admin; }
        get self_requires_password() { return this.requires_password(exports.app.$._client.username); }
        self_has_access(password) { return exports.app.$._client.is_admin || this.has_access(exports.app.$._client.username, password); }
    ...
    Object.defineProperty(AccessControl, "DEFAULT_ACCESS_FOR_SELF", {
        get:()=>{
            return {
                "*": { access: "allow" },
                [exports.app.$._client.username] : { access: "owner" },
            };
        }
    });
    ...
    class Remote extends core_js.utils.EventEmitter {
        constructor() {
            super();
            this._changes = [];
        ...
        _debounced_update = core_js.dom_utils.debounce_next_frame(()=>this._update());
        _update() {
            var changes = core_js.utils.tree_from_entries(this._changes);
            core_js.utils.clear(this._changes);
            // !! IMPORTANT FOR DATES AND THINGS LIKE THAT.
            core_js.utils.deep_walk(changes, function(k,v) {
        ...
        _push(...items) {
            this._changes.push(...items.map(i=>core_js.utils.deep_copy(i)));
    ...
    class Target extends DataNode {
        ...
        get _streams() { return exports.app.$._streams.filter(s=>s.targets.find(t=>t.id == this.id)); }
    ...
    class Stream extends DataNode {
        ...
        get _session() {
            return exports.app.$.sessions[this.session_id];
        }
        get _is_running() {
    ...
    class Session extends DataNode {
        [Observer.RESET_KEY]() {
            this.id = "";
            this.type;
            this.index = 0;
        ...
        get _connected_nms_sessions() {
            return Object.values(exports.app.$.nms_sessions).filter(s=>s.publishStreamPath.split("/").pop() === this.id);
        }
    ...
    class PlaylistItem extends DataNode {
        ...
        get _media_info() {
            return exports.app.$.media_info[this.filename];
        }
        get _is_deleted() {
        ...
        *_get_children(track_index=null, recursive=false) {
        ...
        _update_userdata() {
            ...
            if (children.length) {
                ...
                media_duration = this.props.title_duration || exports.app.playlist_item_props_class.title_duration.default;
            } else if (this.filename === "livestreamer://empty") {
                media_duration = this.props.empty_duration || exports.app.playlist_item_props_class.empty_duration.default;
            }
            ...
            if (is_playlist) {
                ...
                chapters = core_js.utils.deep_copy(media_info.chapters || EMPTY_ARRAY);
            }
            if (chapters) {
                var min = 0;
        ...
        get _media_infos() {
            return this._info.filenames.map(f=>exports.app.$.media_info[f]).filter(mi=>mi);
        }
        get _is_processing() {
            var info = this._info;
            if (!info) return false;
            return info.filenames.some(f=>exports.app.$.media_info[f] && exports.app.$.media_info[f].processing) || this._children.some(i=>i._is_processing);
        }
        get _parents() {
        ...
        get _is_navigatable() {
            ...
            if ((!core_js.utils.is_uri(this.filename) || this.filename.startsWith("file://")) && IS_ELECTRON) return true;
            return false;
        }
        get _url() {
            return core_js.utils.is_uri(this.filename) ? new URL(this.filename) : new URL("file://"+this.filename);
        }
        get _download() {
            return exports.app.$.downloads[this.id];
        }
        get _upload() {
            return exports.app.$.uploads[this.id];
        }
        get _active_upload() {
        ...
        get _elfinder_hash() {
            return exports.app.filename_to_elfinder_hash(this.filename);
        }
        ...
        _get_pretty_name(opts) {
            ...
            if (!opts.ext) {
                filename = core_js.utils.split_ext(filename)[0];
            }
            return filename;
        }
        _reveal() {
            ...
            exports.app.playlist.open(next, [this]);
        }
        /** @return {Iterable<PlaylistItem>} */
        *_get_parents(until=null) {
        ...
        _copy(include_non_enumerable=false) {
            if (include_non_enumerable) return Object.fromEntries((core_js.utils.get_property_keys(this).map(k=>[k,core_js.utils.deep_copy(this[k])])));
            return core_js.utils.deep_copy(this);
        }
    ...
    const NULL_CLIENT = Object.freeze(new Client());
    const NULL_SESSION = Object.freeze(new Session());
    const NULL_PLAYLIST_ITEM = Object.freeze(new PlaylistItem());
    const NULL_STREAM = Object.freeze(new Stream());

    class CropPreview extends core_js.utils.EventEmitter {
        add_legend(name, clazz) {
            var elem = core_js.$(`<div><div class="${clazz}" style="width:15px;height:15px"></div><span>${name}</span></div>`)[0];
        ...
        constructor(url, rect, rect2, editable) {
            ...
            this.elem = core_js.$(`<div class="crop-preview-wrapper"><div class="crop-preview"><div class="crop-edges"></div><div class="detected-crop-border"></div><div class="crop-border"></div><img src="${url}" draggable="false"></div></div>`)[0];
            ...
            this.legend_elem = core_js.$(`<div class="legend"></div>`)[0];
            if (editable) {
                if (rect2) {
                ...
                corners.forEach((key,i)=>{
                    var corner_elem = core_js.$(`<div class="drag-corner ${key}"></div>`)[0];
                    corner_elem.style.cursor = corner_cursors[i];
                    this.corners[key] = corner_elem;
                    this.content_elem.appendChild(corner_elem);
                    var parts = key.split("-");
                    
                    core_js.$(corner_elem).on("mousedown", ()=>{
                        var onmousemove = (e)=>{
                            ...
                            core_js.$(corner_elem).css({
                                left: `${core_js.utils.clamp((e.clientX-client_rect.left)/this.content_elem.offsetWidth,min_x, max_x)*100}%`,
                                top: `${core_js.utils.clamp((e.clientY-client_rect.top)/this.content_elem.offsetHeight,min_y, max_y)*100}%`,
                            });
                            ...
                            var rect = new core_js.utils.Rectangle(o);
                            this.update(rect);
                        };
                        core_js.$(document).on("mousemove", onmousemove);
                        core_js.$(document).one("mouseup", (e)=>{
                            core_js.$(document).off("mousemove", onmousemove);
                            this.update(this.rect.clone().fix());
                            this.elem.dispatchEvent(new Event("change"));
                        });
                    });
                });
                this.info_elem = core_js.$(`<div class="info"></div>`)[0];
                this.elem.appendChild(this.info_elem);
                var button_container = core_js.$(`<div style="display:flex; flex-direction:row"></div>`)[0];

                var reset_button = core_js.$(`<button class="button" style="flex:1">Set to Detected Crop</button>`)[0];
                reset_button.addEventListener("click", ()=>{
                    this.update(this.orig_rect);
                });
                button_container.appendChild(reset_button);
                var reset_button2 = core_js.$(`<button class="button" style="flex:1">Set to Current Crop</button>`)[0];
                reset_button2.addEventListener("click", ()=>{
                    this.update(this.orig_crop_rect);
                });
                button_container.appendChild(reset_button2);

                this.save_button = core_js.$(`<button class="button" style="flex:1">Apply Crop</button>`)[0];
        ...
        update(r){
            r = new core_js.utils.Rectangle(r);
            this.rect = r;
            this.fixed_rect = r.clone().fix();
            for (var k in this.corners) {
                var parts = k.split("-");
                core_js.$(this.corners[k]).css({
                    top:`${r[parts[0]]*100}%`,
                    left:`${r[parts[1]]*100}%`,
                });
            }
    ...
    class SelectableList extends core_js.utils.EventEmitter {
        ...
        constructor(elem, options) {
            ...
            window.addEventListener("keydown", this.on_keydown = (e)=>{
                if (!core_js.dom_utils.has_focus(this.elem)) return;
                e.preventDefault();
                ...
                index = core_js.utils.clamp(index, 0, items.length-1);
    ...
    class JSONContainer extends UI {
        constructor(data, collapsed_children=false) {
    ...
    class ModalPropertyContainer extends UI.PropertyContainer {
        get showing() { return !!this.fb; }
        get changes() { return Object.keys(core_js.utils.deep_diff(this.property_lookup_on_show, this.property_lookup)); }
        ...
        constructor(settings) {
            ...
            this.header = new UI(core_js.$(`<div class="modal-header"></div>`)[0]);
            this.content = new UI(core_js.$(`<div class="modal-content"></div>`)[0]);
            this.footer = new UI(core_js.$(`<div class="modal-footer"></div>`)[0]);
    ...
    class LocalServerTargetConfigMenu extends ModalPropertyContainer {
        constructor() {
            super({
                "modal.title": `Configure Local Media Server`,
                "modal.title-overflow": true,
                data: ()=>core_js.utils.try(()=>exports.app.$._session.target_configs["local"]) || {},
            });
    ...
    class UserConfigurationSettings extends ModalPropertyContainer {
        constructor() {
            super({
                data:()=>exports.app.$.settings,
                "modal.title": "Client Configuration",
                "modal.footer":true,
            });

            function get_default() { return exports.app.settings.defaults[this.name] }
            ...
            var reset_layout_button = new UI.Button(`Reset Layout`, {
                click: ()=>{
                    exports.app.settings.set("layout", null);
                    exports.app.update_layout();
                }
            });
            ...
            this.on("property-change", (e)=>{
                ...
                exports.app.settings.set(e.name, e._value);
            });
        }
    }
    class KeyboardShortcuts extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            var sections = {
                ...
                "Playlist": [
                    ...
                    ...exports.app.playlist.all_commands.filter(c=>c.options.view === PLAYLIST_VIEW.LIST && c.shortcut()).map(c=>[c.shortcut_alt(), c.description_or_name()])
                ],
                "Playlist (Timeline Mode)": [
                    [`[Arrow Left] / [Arrow Right]`, `Select Previous / Next Item`],
                    [`Mouse Wheel Up / Down`, `Zoom In & Out`],
                    [`Left Click Tick Bar`, `Place Timeline Cursor`],
                    ...exports.app.playlist.all_commands.filter(c=>c.options.view === PLAYLIST_VIEW.TIMELINE && c.shortcut()).map(c=>[c.shortcut_alt(), c.description_or_name()])
                ]
            };
    ...
    class FileSystemInfoMenu extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            var init = async()=> {
                for (let id in exports.app.$.volumes) {
                    if (exports.app.$.volumes[id].driver !== "LocalFileSystem") continue;
                    var volume = exports.app.$.volumes[id];
                    var loading_el = core_js.$(`<tr><td colspan="6"><i class="fas fa-sync fa-spin"></i> Loading...</td></tr>`)[0];
                    core_js.dom_utils.empty(tbody);
                    tbody.append(loading_el);
                    var r = await exports.app.request({
            ...
            var render = (node)=>{
                ...
                size_el.innerText = core_js.utils.format_bytes(node.size);
    ...
    class SystemManagerMenu extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            class Bar extends UI {
                constructor(settings) {
                    ...
                    this.bar = new UI(core_js.$(`<div class="bar"></div>`)[0]);
                    ...
                    this.on("update", ()=>{
                        ...
                        var inner = core_js.$(`<div class="inner"></div>`)[0];
                        ...
                        var text = core_js.$(`<div class="text">${str}</div>`)[0];
            ...
            class Process extends UI.Column {
                constructor(name) {
                    ...
                    var is_running = ()=>exports.app.$.processes[name].status == "online";
                    ...
                    var restart_button = new UI.Button("RESTART", {
                        click: ()=>{
                            exports.app.request_no_timeout({
                    ...
                    this.on("update", ()=>{
                        var conf_name = exports.app.$.processes[name]["title"];
                        var conf_desc = exports.app.$.processes[name]["description"];
                        var p = exports.app.$.processes[name];
                        var color = null;
                        ...
                        var pinfo = exports.app.$.process_info[p.pid] || {};
                        var cpu = Number((pinfo.cpu||0)*100).toLocaleString(undefined, {minimumFractionDigits:2,maximumFractionDigits:2})+"%";
                        var mem = core_js.utils.format_bytes(pinfo.memory||0);
                        var uptime = core_js.utils.ms_to_human_readable_str(pinfo.elapsed||0);
                        var s = {"CPU":cpu,"Memory":mem,"Transfer rate":`↑ ${core_js.utils.format_bytes(pinfo.sent)}ps / ↓ ${core_js.utils.format_bytes(pinfo.received)}ps`, "Uptime":uptime};
            ...
            var uptime = this.content.append(new UI({
                "update":()=>{
                    set_inner_html(uptime.elem, `System uptime: ${core_js.utils.ms_to_human_readable_str(exports.app.$.sysinfo.uptime*1000)}`);
                }
            }));
            var transfer = this.content.append(new UI({
                "update":()=>{
                    set_inner_html(transfer.elem, `Transfer rate: ↑ ${core_js.utils.format_bytes(exports.app.$.sysinfo.sent)}ps / ↓ ${core_js.utils.format_bytes(exports.app.$.sysinfo.received)}ps`);
                }
            }));
            this.content.append(new Bar({
                label: "Disk",
                value: ()=>exports.app.$.sysinfo.disk_total-exports.app.$.sysinfo.disk_free,
                total: ()=>exports.app.$.sysinfo.disk_total,
                format: (x)=>core_js.utils.format_bytes(x)
            }));
            this.content.append(new Bar({
                label:"Memory",
                value: ()=>exports.app.$.sysinfo.memory_total-exports.app.$.sysinfo.memory_free,
                total: ()=>exports.app.$.sysinfo.memory_total,
                format: (x)=>core_js.utils.format_bytes(x)
            }));
            this.content.append(new Bar({
                label:"CPU Usage",
                value: ()=>exports.app.$.sysinfo.cpu_avg,
            ...
            this.on("update", ()=>{
                var processes = Object.keys(exports.app.$.processes);
                core_js.utils.sort(processes, (p)=>p==="main"?0:1);
                core_js.dom_utils.rebuild(process_container.elem, processes, {
                    id_callback: (p)=>p,
                    add: (p, elem, i)=>{
    ...
    class FileManagerMenu extends ModalPropertyContainer {
        constructor() {
            super({
                "modal.footer": false,
                "modal.header": false,
            ...
            this.iframe = core_js.$(`<iframe allowfullscreen="allowfullscreen" allow="autoplay; fullscreen" scrolling="auto" width="100%" height="100%" frameBorder="0"></iframe>`)[0];
    ...
    class ScheduleGenerator extends ModalPropertyContainer {
        constructor() {
            super({
                "modal.title": "Schedule Generator",
                data: ()=>exports.app.settings.get("schedule_generator"),
            ...
            row.append(
                ...
                this.min_duration_filter = new UI.Property("min_duration_filter", "Minimum Duration Filter", `<select>`, {
                    default: 0,
                    options: [[0,"None"],...[10,30,1*60,2*60,5*60,10*60].map(f=>[f, core_js.utils.seconds_to_human_readable_str(f)])],
            ...
            this.on("property-change", (e)=>{
                if (!e.name || !e.trigger) return;
                exports.app.settings.set("schedule_generator", this.named_property_lookup_not_null);
            });

            this.on("update", (e)=>{
                var day = 60 * 60 * 24;
                var t = this.start_time.value;
                var time = core_js.utils.timespan_str_to_seconds(t, "hh:mm");
                ...
                var add_line = (name)=>{
                    ...
                    time_r %= day;
                    var time_str = core_js.utils.seconds_to_timespan_str(time_r , "hh:mm");
                ...
                exports.app.$._session.playlist["0"]._children.forEach(c=>walk(c));
                add_line(`Fin`);
    ...
    class FontSettings extends ModalPropertyContainer {
        constructor() {
            super({
                data:()=>exports.app.$.fonts,
            ...
            var left_elem = core_js.$(`<div class="left"></div>`)[0];
            var list = new SelectableList(core_js.$(`<div class="content thin-scrollbar"></div>`)[0], {});
            var left_footer_elem = core_js.$(`<div class="footer"></div>`)[0];
            ...
            var right_elem = core_js.$(`<div class="right"></div>`)[0];
            var info_elem = core_js.$(`<div class="content thin-scrollbar"></div>`)[0];
            var right_footer_elem = core_js.$(`<div class="footer"></div>`)[0];
            ...
            fd.on("drop", (files)=>{
                exports.app.upload_queue.add(files, {dir:"/fonts"});
            });
            
            var add_button = new UI.Button("Add New Font", {
                click:async ()=>{
                    var files = core_js.dom_utils.upload(`application/font-sfnt,application/font-sfnt`, true);
                    exports.app.upload_queue.add(files, {dir:"/fonts"});
                },
    ...
    class SplitSettings extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            this.time_list.input_modifiers.push(v=>{
                return [...v.split(delimiter_regex)].map(v=>core_js.utils.timespan_str_to_seconds(v)).filter(valid_time)
            });
            this.time_list.output_modifiers.push(v=>{
                if (v.length == 1 && !v[0]) return "";
                return v.map(v=>core_js.utils.seconds_to_timespan_str(v, "h?:mm:ss.SSS")).join(", ")
            });
            ...
            row.append(
                this.seek = new SeekBar({
                    "label": ()=>`Timeline [${core_js.utils.seconds_to_timespan_str(this.seek.get_setting("seek.duration"), exports.app.user_time_format)}]`,
            ...
            this.seek.on("markers-change", ()=>{
                ...
                times = core_js.utils.sort(times);
                this.time_list.set_value(times);
            });
            
            this.split_button = new UI.Button(`Split`, {
                click: ()=>{
                    exports.app.playlist_split(this.datas, this.get_splits(), true);
                    this.hide();
                }
            });
    ...
    class ScheduleStreamSettings extends ModalPropertyContainer {
        constructor() {
            super({
                data: ()=>exports.app.$._session,
            ...
            row.append(
                this.schedule_start_time = new UI.DateTimeProperty("schedule_start_time", null, {
                    "label": function(){
                        ...
                        if (this.value) {
                            n += ` (<i>${core_js.utils.time_diff_readable(Date.now(), new Date(this.value))}</i>)`;
                        }
                        return n;
                    },
                    "default": null,
            ...
            this.on("property-change", (e)=>{
                if (!e.name || !e.trigger) return;
                exports.app.$._push([`sessions/${exports.app.$._session.id}/${e.name}`, e._value]);
                exports.app.request({
    ...
    class SessionConfigurationSettings extends ModalPropertyContainer {
        constructor() {
            super({
                data: ()=>exports.app.$._session,
            ...
            function get_default() { return core_js.utils.try(()=>exports.app.$.properties[this.name].default); }
            ...
            this.stream_host = new UI.Property(null, "Stream Host", `<input type="text" readonly>`, {
                "default": exports.app.get_rtmp_url(),
                "reset": false,
                "copy": true,
            ...
            this.stream_key = new UI.Property(null, "Stream Key", `<input type="text" readonly>`, {
                "default": ()=>`private/${exports.app.$._session.id}`,
                "reset": false,
                "copy": true,
            });
            ...
            [this.background_mode, this.background_color, this.background_file, this.background_file_start, this.background_file_end] = create_background_properties({
                ...
                "options": ()=>core_js.utils.try(()=>exports.app.$.properties.background_mode.options, []),
                "default": ()=>core_js.utils.try(()=>exports.app.$.properties.background_mode.default, null),
            }, true);
    ...
    class AdminSettings extends ModalPropertyContainer {
        constructor() {
            super({
                data: ()=>exports.app.$._session,
                "modal.title":"Admin",
    ...
    class ChangeLog extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            this.on("show",()=>{
                ...
                exports.app.settings.set("last_change_log", exports.app.$.change_log.mtime);
                set_inner_html(this.content.elem, `<div>${exports.app.$.change_log.html}</div>`);
            });
        }
    }

    class UploadsDownloadsMenu extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            this.on("update", ()=>{
                var content = new UI();
                var stats = types.map(t=>`Total ${t.slice(0,-1)} rate: ${core_js.utils.format_bytes(core_js.utils.sum(Object.values(exports.app.$[t]).map(u=>u.speed)))+"ps"}`).join(" | ");
                content.append(...core_js.$(`<div>${stats}</div>`));
                ...
                for (var type of types) {
                    ...
                    var header = {
                        ...
                        "bytes":{
                            name: core_js.utils.capitalize(type.slice(0,-1))+"ed",
                            style: "white-space: nowrap",
                        },
                        "total":{
                            name: "Total",
                    ...
                    for (var [id,u] of Object.entries(exports.app.$[type])) {
                        rows.push({
                            id,
                            ...u,
                            rate: core_js.utils.format_bytes(u.speed)+"ps",
                            bytes: core_js.utils.format_bytes(u.bytes),
                            total: core_js.utils.format_bytes(u.total),
                            progress: `${((u.bytes/u.total)*100).toLocaleString(undefined, {maximumFractionDigits:2,minimumFractionDigits:2})}%`
                        });
                    }
                    var table = core_js.dom_utils.build_table(rows, { header, empty: `No active ${type}` });
                    content.append(table);
                }
                core_js.dom_utils.sync_dom(this.content.elem, content.elem, {attrs:false});
            });
        }
    }

    class JSONViewer extends ModalPropertyContainer {
    ...
    class InfoSettings extends JSONViewer {
        /** @param {PlaylistItem[]} items */
        async show(items) {
            var name;
    ...
    class SetTimePosSettings extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            row.append(
                this.chapter_select = new UI.Property(null, "Chapter", `<select>`, {
                    "hidden":()=>!exports.app.settings.get("show_chapters") || exports.app.$._session._current_chapters.length < 2,
                    "options":()=>{
                        return exports.app.$._session._current_chapters.map((c,i)=>[i, exports.app.chapter_to_string(c, true)])
                    },
                    "reset":false,
                }),
            ...
            this.time_pos.on("change",(e)=>{
                if (!e.trigger) return;
                this.chapter_select.set_values((exports.app.$._session._get_current_chapter_at_time(e._value)||EMPTY_OBJECT).index);
            });
            this.chapter_select.on("change",(e)=>{
                if (!e.trigger) return;
                var c = exports.app.$._session._current_chapters[e._value];
                this.time_pos.set_values(c.start);
            });

            this.ok = new UI.Button("Seek", {
                click: ()=>{
                    exports.app.seek(this.time_pos.value);
                    this.hide();
                }
            });
            ...
            this.on("show",()=>{
                this.time_pos.settings.default = exports.app.$._session._current_time;
                this.time_pos.reset(true);
            });
        }
    }

    class SetVolumeSettings extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            volume_input.set_value(exports.app.$._session.volume_target);
            vol_speed.set_value(exports.app.$._session.volume_speed);

            this.ok = new UI.Button("Apply Changes", {
                disabled: ()=>this.changes.length==0,
                click: ()=>{
                    exports.app.media_player.volume.set_value(volume_input.value, {trigger:true});
                    exports.app.media_player.vol_speed.set_value(vol_speed.value, {trigger:true});
                    this.hide();
                }
            });
            this.cancel = new UI.Button("Cancel", {
    ...
    class ExternalSessionConfigurationMenu extends ModalPropertyContainer {
    ...
    class TargetEditMenu extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            this.name.add_validator(UI.VALIDATORS.not_empty, (v)=>{
                return Object.values(exports.app.$.targets).filter((t)=>t!=this.data).map(t=>t.name).includes(v) ? "Name already exists." : true
            });
            ...
            var save_button = new UI.Button(`Save`, {
                ...
                click: ()=>{
                    exports.app.request({
                        ...
                        arguments: [{access_control:{[exports.app.$._client.id]:{"access":"owner"}} , ...this.named_property_lookup}]
                    });
                    this.hide();
                }
            });
    ...
    class TargetConfigurationMenu extends ModalPropertyContainer {
        ...
        get targets_remaining() {
            ...
            core_js.utils.sort(targets, t=>!t.locked, t=>t.ts);
            return targets;
        }
        /** @param {TargetsProperty} parent_prop */
        constructor(parent_prop, settings) {
            ...
            var onchange;
            this.on("show", ()=>{
                exports.app.on("change", onchange = ($)=>{
                    if ($.targets) update_targets();
                });
                update_targets();
            });
            this.on("hide", ()=>{
                exports.app.off("change", onchange);
            });
            var update_targets = ()=>{
                this.targets = {...exports.app.$.targets};
            ...
            var rebuild = ()=>{
                if (parent_prop) {
                    core_js.dom_utils.rebuild(selected_el, this.targets_selected, { add });
                ...
                core_js.dom_utils.rebuild(list_el, this.targets_remaining, { add });
            ...
            var add = (target, elem, i)=>{
                ...
                var label_el = core_js.$(`<span></span>`)[0];

                var checkbox_input = core_js.$(`<input type="checkbox">`)[0];
                var text_wrapper_elem = core_js.$(`<div class="text-wrapper"></div>`)[0];
                ...
                var get_config_menu = ()=>core_js.utils.try(()=>exports.app.target_config_menus[target.id]);
                ...
                var name_elem = core_js.$(`<span class="name"></span>`)[0];
                var description_elem = core_js.$(`<div class="description"></div>`)[0];
                ...
                set_inner_html(description_elem, core_js.utils.convert_links_to_html(target.description || ""));
    ...
    class TargetsProperty extends UI.Property {
        constructor(name, label, settings) {
            ...
            var input = core_js.$(`<input type="text" readonly>`)[0];
            ...
            this.output_modifiers.push(v=>v.length ? v.length == 1 ? (exports.app.$.targets[v[0]]||EMPTY_OBJECT).name : `${v.length} Targets` : `None`);
        }
    }
    class SeekBar extends UI {
        constructor(settings) {
            var input = core_js.$(
`<div class="seek-wrapper">
    ...
    <span id="time-left">00:00:00</span>
            ...
            this.ticks_bar = new TicksBar(this.ticks_bar_elem, {
                ...
                modifier: (html, t)=>{
                    ...
                    if (curr_chapters.length) {
                        html = curr_chapters.map(c=>`<div class="chapter">${exports.app.chapter_to_string(c)}</div>`).join("") + html;
                    }
                    return html;
                }
            });

            var hover_listener = new core_js.dom_utils.TouchListener(this.seek_elem, {
                mode: "hover",
                start: (e)=>{
            ...
            var seek_listener = new core_js.dom_utils.TouchListener(this.seek_elem, {
                start: (e)=>{
            ...
            new core_js.dom_utils.TouchListener(this.seek_elem, {
                start: (e)=>{
                    this.seek_elem.focus();
                    var data = this.ticks_bar.parse_event(e);
            ...
            this.on("update", ()=>{
                ...
                if (this._ranges_hash != ranges_hash) {
                    ...
                    core_js.dom_utils.empty(this.ranges_elem);
                    if (duration && ranges) {
                        for (var r of ranges) {
                            var e = core_js.$(`<div class="range"></div>`)[0];
                ...
                if (this._markers_hash != markers_hash) {
                    ...
                    core_js.dom_utils.empty(this.markers_elem);
                    if (duration) {
                        for (var m of markers) {
                            var e = core_js.$(`<div class="marker"><div></div></div>`)[0];
                ...
                if (this._chapters_hash != chapters_hash) {
                    ...
                    core_js.dom_utils.empty(this.chapters_elem);
                    if (duration && chapters.length > 1) {
                        chapters.forEach((c,i)=>{
                            var d = Math.max(0, c.end-c.start);
                            var e = core_js.$(`<div class="chapter"></div>`)[0];
                ...
                set_text(this.time_elem, `${core_js.utils.seconds_to_timespan_str(time, exports.app.user_time_format)}`);
                var tl = "";
                var tlm = this.get_setting("seek.time_left_mode");
                if (tlm === TimeLeftMode.TIME_LEFT) tl = `-${core_js.utils.seconds_to_timespan_str(Math.max(0, time_left), exports.app.user_time_format)}`;
                else if (tlm === TimeLeftMode.DURATION) tl = core_js.utils.seconds_to_timespan_str(Math.max(0, duration), exports.app.user_time_format);
        ...
        remove_marker(m) {
            var markers = this.get_setting("seek.markers");
            core_js.utils.array_remove(markers, m);
            this.update_settings({"seek.markers": markers});
        }
    }
    class MediaSeekBar extends SeekBar {
        constructor() {
            super();
        
            var last_time;
            this.on("seek-end", (e)=>{
                if (!exports.app.media.do_live_seek || last_time != e.time) exports.app.seek(e.time);
            });
            this.on("seeking", (e)=>{
                if (exports.app.media.do_live_seek) exports.app.seek(e.time);
                last_time = e.time;
            });
            this.on("time_left_mode", (v)=>{
                exports.app.settings.set("time_left_mode", v);
            });
            var update_time_left_mode = ()=>{
                this.update_settings({
                    "seek.time_left_mode": exports.app.settings.get("time_left_mode")
                });
            };
            exports.app.settings.on("change", (e)=>{
                if (e.name === "time_left_mode") update_time_left_mode();
            });
            update_time_left_mode();
            this.on("pre_update", ()=>{
                var running = exports.app.$._stream._is_running;
                var seek_pause = this.get_setting("seek.seek_pause");
                if (exports.app.$._stream.mpv.seeks != this._last_seeks || !running) {
                    seek_pause = false;
                }
                this._last_seeks = exports.app.$._stream.mpv.seeks;
                var buffering = running && (exports.app.media.buffering || seek_pause || !exports.app.$._stream.mpv.preloaded);

                Object.assign(this.settings, {
                    "seek.time": exports.app.media.time,
                    "seek.seekable": exports.app.media.seekable,
                    "seek.duration": exports.app.media.duration,
                    "seek.chapters": exports.app.media.chapters,
                    "seek.ranges": exports.app.$._session._current_seekable_ranges,
    ...
    class StreamKeyGeneratorSettings extends UI.PropertyContainer {
        constructor() {
            super({});
            ...
            this.stream_name = new UI.Property(null, "Name", `<input type="text">`, {
                "default": ()=>`${exports.app.$._client.username}'s Stream`,
            ...
            var update_output = ()=>{
                ...
                var host = exports.app.get_rtmp_url();
                this.output_host.set_values(host);
                var key = `livestream/${exports.app.$._client.username}`;
                if (query_str) key += "?"+query_str;
                this.output_key.set_values(key);
            };

            var debounced_update_output = core_js.dom_utils.debounce_next_frame(()=>update_output());
            this.on("property-change",(e)=>{
                if (!e.trigger) return;
    ...
    class StreamConfigurationMenu extends ModalPropertyContainer {
        constructor(){
            super({
                "modal.title": "Stream Configuration",
                data:()=>exports.app.$._stream,
            });
            ...
            var stream_targets = new TargetsProperty("targets", "Stream Target(s)", {
                "hidden": ()=>exports.app.$._stream.method != "rtmp",
                "auto_apply": false,
            });
    ...
    class HandoverSessionMenu extends ModalPropertyContainer {
        constructor(){
            super({
            ...
            var handover_stream_property = new UI.Property(null, "Session", `<select>`, {
                "options":()=>exports.app.get_handover_sessions_options(),
                "reset":false,
    ...
    class SavePlaylistSettings extends ModalPropertyContainer {
        constructor() {
            super({
                "modal.title": ()=>`Save Playlist '<span>${playlist_name}</span>'`,
                "modal.title-overflow": true,
                "modal.footer": true,
                data: ()=>exports.app.settings.get("save_playlist_settings"),
            });
            
            this.on("property-change", (e)=>{
                if (!e.name || !e.trigger) return;
                exports.app.settings.set("save_playlist_settings", this.named_property_lookup_not_null);
            });
            ...
            this.preview = new UI.Property(null, "Preview", `<div class="text-block"></div>`, {
                "update":()=>{
                    var hash = JSON.stringify([exports.app.playlist.current.id, ...[this.playlist_save_format, this.playlist_save_children, this.playlist_json_spaces].map(p=>p.value)]);
            ...
            var serialize = ()=>{
                ...
                var process = (item)=>{
                    ...
                    if (!core_js.utils.is_empty(item.props)) o.props = item.props;
                ...
                var items = exports.app.playlist.current._children.map(i=>process(i));
            ...
            this.on("show", ()=>{
                playlist_name = exports.app.playlist.current._get_pretty_name() || exports.app.$._session.name;
                filename = `${core_js.utils.sanitize_filename(playlist_name)}-${core_js.utils.date_to_string()}.json`;
                render_preview();
            });
    ...
    class HistorySettings extends ModalPropertyContainer {
        history = [];
        constructor() {
            super({
            ...
            var table_data = {
                "Time":(data)=>{
                    ...
                    var e = core_js.$(`<span>${core_js.utils.time_diff_readable(new Date(), mtime)}</span>`)[0];
            ...
            var row = this.content.append(new UI.FlexRow());
            var table_wrapper = core_js.$(`<div class="table-wrapper thin-scrollbar"></div>`)[0];
            var table = core_js.$(`<table><thead></thead><tbody></tbody></table>`)[0];
            var thead = table.querySelector("thead");
            var thead_tr = core_js.$(`<tr></tr>`)[0];
            var tbody = table.querySelector("tbody");
            Object.keys(table_data).forEach(k=>{
                core_js.$(thead_tr).append(`<th>${k}</th>`);
            });
            ...
            var info_wrapper_elem = core_js.$(`<div class="info-wrapper"></div>`)[0];
            var info_elem = core_js.$(`<div class="info thin-scrollbar"></div>`)[0];
            info_wrapper_elem.append(info_elem);
            var info_footer_elem = core_js.$(`<div class="footer"></div>`)[0];
            ...
            this.selectable_list.on("change", (item, i)=>{
                ...
                core_js.dom_utils.empty(info_elem);
                ...
                for (var k of ["curr", "prev"]) {
                    ...
                    var box = core_js.$(`<div><h3>${title}</h3><ul></ul></div>`)[0];
                    info_elem.append(box);
                    var ul = box.querySelector("ul");
                    if (!entries || entries.length == 0) {
                        core_js.$(ul).append(`<li>No changes.</li>`);
                    } else {
                        entries.forEach(([path,v])=>{
                            ...
                            core_js.$(ul).append(`<li><i>[${path.join("•")}]</i>\n<strong>(${from}) => (${to})</strong></li>`);
                        });
                    }
                }
            });
    ...
    class PlaylistAddURLMenu extends ModalPropertyContainer {
        constructor() {
            super({
            ...
            var row = this.content.append(new UI.FlexRow());
            row.append(...core_js.$(`<span>If you're having problems downloading some media it might be due to geo-blocking in the server's locale, try <a href="https://oleksis.github.io/youtube-dl-gui/" target="_blank">yt-dlg</a> to download the media in your locale and upload to the server.</span>`));
    ...
    class PlaylistModifySettings extends ModalPropertyContainer {
        /** @param {PlaylistItem[]} items */
        show(items, new_type) {
        ...
        constructor() {
            super({
                ...
                "modal.close": ()=>{
                    ...
                    if (exports.app.$._session._is_running && this.datas.some(d=>d === exports.app.$._session._current_playing_item) && this.changes.length) {
                        exports.app.prompt_for_reload_of_current_item();
                    }
                    return true
                },
                ...
                this.reset_button = new UI.Button("Reset", {
                    ...
                    click:()=>{
                        if (this.is_new) {
                            ...
                            exports.app.$._push(...this.datas.map(data=>[`sessions/${exports.app.$._session.id}/playlist/${data.id}/props`,{[Observer.RESET_KEY]:"Object"}]));
                        }
                    }
                })
            );
            ...
            this.on("property-change", (e)=>{
                ...
                if (e.name && e.trigger) {
                    exports.app.playlist_update(e.datas.map(data=>[`${data.id}/${e.name}`, e._value]));
            ...
            var update_layout_next_frame = core_js.dom_utils.debounce_next_frame(()=>update_layout());
        }
    }

    class MediaSettingsInterface {
        ...
        constructor(parent) {
            ...
            let get_default_stream_id = (streams, type)=>{
                ...
                return streams.indexOf(core_js.utils.get_default_stream(streams,type))+1;
            };
            ...
            let get_streams = (item, type)=>{
                var streams;
                if (this.is_parent_modify) streams = core_js.utils.try(()=>item.media_info.streams);
                else streams = core_js.utils.try(()=>exports.app.$._session._current_playing_item._media_info.streams);
            ...
            let get_default = function() {
                ...
                if (_this.is_parent_modify || parent._mode === MediaSettingsMode.current) {
                    value = core_js.utils.get(exports.app.$._session.player_default_override, name);
                }
                if (value === undefined) {
                    value = core_js.utils.try(()=>exports.app.get_property(`playlist/*/props/${name}`).default);
                }
            ...
            let get_options = function(){
                ...
                var name = this.name.split("/").pop();
                if (_this.is_parent_modify) {
                    options = core_js.utils.try(()=>exports.app.get_property(`playlist/*/props/${name}`).options);
                }
                if (options === undefined) {
                    options = core_js.utils.try(()=>exports.app.get_property(`player_default_override/${name}`).options);
                }
                return options || [];
            };
            ...
            this.volume_normalization = new UI.Property(prop_name("volume_normalization"), "Volume Normalization", `<select></select>`, {
                "options":()=>{
                    return [[false,"Off"], ...(core_js.utils.try(()=>exports.app.$.properties.player_default_override.props.volume_normalization.options)||EMPTY_ARRAY).map(([f,_])=>[f,f])]
                },
                "default": get_default,
            });
            ...
            if (this.is_parent_modify) {
                ...
                this.clip_end = new UI.TimeSpanProperty(prop_name("clip_end"), null, {
                    "label": ()=>core_js.dom_utils.is_visible(this.clip_start.elem) ? "Clip End" : "Duration",
                    "timespan.format": "h:mm:ss.SSS",
                    "min": 0,
                    "max": default_duration,
                ...
                function get_background_options(item) {
                    var options = core_js.utils.deep_copy(get_options.apply(this));
                ...
                [this.background_mode, this.background_color, this.background_file, this.background_file_start, this.background_file_end] = create_background_properties({
                    ...
                    "options":function(item){
                        ...
                        if (item.filename == "livestreamer://intertitle") {
                            ...
                            var audio_display_default_opt = core_js.utils.try(()=>exports.app.$.properties.background_mode.options.find(o=>o[0]==exports.app.$._session.background_mode));
                ...
                var get_current_crop_rect = ()=>{
                    return new core_js.utils.Rectangle({left:this.crop_l.value, top:this.crop_t.value, right:1-this.crop_r.value, bottom:1-this.crop_b.value});
                };
                ...
                function show_crop_edit_modal(id, index) {
                    var data = exports.app.$._session.detected_crops[id];
                ...
                (()=>{
                    ...
                    var timeline_elem = core_js.$(`<div class="timeline"><div></div></div>`)[0];
                    ...
                    var black_overlay = core_js.$(`<div></div>`)[0];
                    ...
                    var container = core_js.$(`<div class="preview-container"></div>`)[0];
                    ...
                    var outline_elem = core_js.$(`<div class="preview-text"></div>`)[0];
                    container.append(outline_elem);
                    var text_elem = core_js.$(`<div class="preview-text"></div>`)[0];
                    ...
                    var restart_animation = ()=>{
                        ...
                        if (!title_preview_style) {
                            title_preview_style = core_js.$(`<style></style>`)[0];
                            exports.app.body_elem.append(title_preview_style);
                        }
                        let style_text = `
                    @keyframes title-preview-timeline {
                        ...
                        core_js.dom_utils.restart_animation(elem);
                    };
                ...
                this.label = new UI.Property(prop_name("label"), "Label", `<input type="text">`, {
                    /** @param {PlaylistItem} item */
                    "default": (item)=>{
                        return item._get_pretty_name({label:false}) || "";                },
                });

                this.color = new UI.Property(prop_name("color"), "Item Color", `<select></select>`,{
                    "options": Object.keys(item_colors).map(k=>{
                        return {value:k, text:core_js.utils.capitalize(k), style:{"background-color":item_colors[k]||"#fff"}};
                    }),
                    "update":function() {
        ...
        get_layout() {
            ...
            this.parent._new_type === "rtmp" || types.every(t=>t==="rtmp");
            var is_normal = !this.parent._new_type && types.every(t=>t==="normal");
            
            items.every(i=>i.num_tracks == 2);
            var is_merged_playlist = items.every(i=>i.is_merged_playlist);
            var is_parent_merged = items.every(i=>i.root_merged_playlist);
            var is_youtube = items.every(i=>core_js.utils.try(()=>i.media_info.probe_method === "youtube-dl"));
            var is_image = items.every(i=>core_js.utils.try(()=>i.media_info.duration <= IMAGE_DURATION));
            items.every(i=>core_js.utils.try(()=>i.media_info.exists));
    ...
    class FileProperty extends UI.Property {
        constructor(name, label, settings = {}) {
            var input = core_js.$(`<input type="text" class="file" readonly>`)[0];
    ...
    class EditAccessControlMemberMenu extends ModalPropertyContainer {
        /** @param {AccessControlProperty} prop */
        constructor(prop) {
            var is_new = ()=>core_js.utils.is_empty(this.data);
            super({
            ...
            this.suspended = new UI.Property("suspended", "Suspended", `<select>`, {
                "default": false,
                "options": YES_OR_NO,
                "disabled": ()=>this.data.username != "*" && this.access.value === "owner" && this.data.username === exports.app.$._client.username,
    ...
    class AccessControlProperty extends UI.Property {
        constructor(name, label, settings = {}) {
            var elem = core_js.$(`<div class="access-control"></div>`)[0];
            ...
            this.debounced_update_value = core_js.dom_utils.debounce_next_frame(()=>this.update_value());

            var columns = {
                "Username": (data)=>core_js.$(`<span>${data.username}</span>`)[0],
                ...
                "Controls": (data)=>{
                    ...
                    if (this.access_control.self_is_owner_or_admin) {
                        edit_button = core_js.$(`<button title="Edit"><i class="fas fa-wrench"></i></button>`)[0];
                        edit_button.onclick = ()=>{
                        ...
                        if (data.username !== "*" && data.access !== "owner") {
                            suspend_button = core_js.$(`<button title="${data.suspended ? "Unsuspend" : "Suspend"}"><i style="opacity:${data.suspended?0.5:1.0};"class="far fa-pause-circle"></i></button>`)[0];
                        ...
                        if (data.username !== "*") {
                            delete_button = core_js.$(`<button title="Delete"><i class="fas fa-trash-alt"></i></button>`)[0];
                    ...
                    var $buttons = core_js.$(`<div class="control-buttons"></div>`);
                    $buttons.append([edit_button, suspend_button, delete_button].filter(e=>!!e));
                    return $buttons[0];
                }
            };
            var table_elem = core_js.$(`<table></table>`)[0];
            var thead_elem = core_js.$(`<thead></thead>`)[0];
            var tbody_elem = core_js.$(`<tbody></tbody>`)[0];
            var tfoot_elem = core_js.$(`<tfoot><tr><td></td></tr></tfoot>`)[0];
            var add_button = core_js.$(`<button class="button" style="width:100%"></button>`)[0];
            ...
            thead_elem.append(core_js.$(`<tr>${Object.keys(columns).map(c=>`<th>${c}</th>`)}</tr>`)[0]);
            var render = ()=>{
                core_js.dom_utils.empty(tbody_elem);
                ...
                for (let user of this.access_control.users) {
                    ...
                    core_js.$(Object.values(columns).map(column_cb=>column_cb(user))).toArray().forEach(c=>{
                        var td = core_js.$(`<td></td>`)[0];
                        core_js.$(td).append(c);
                        tr.append(td);
                    });
            ...
            this.on("change", (e)=>{
                this.access_control.data = core_js.utils.deep_copy(e._value);
                render();
            });
            render();
        }
        ...
        update_value() {
            this.set_values(core_js.utils.deep_copy(this.access_control.data), {trigger:true});
        }
    }

    class RangeProperty extends UI.Property {
        constructor(name, label, settings = {}) {
            var input = core_js.$(`<div class="ui-slider-range"></div>`)[0];
            ...
            super(name, label, input, Object.assign({
                ...
                "spinner": false,
                "setup": ()=>{
                    core_js.noUiSlider.create(input, {
                        start: [0, 1],
                        connect: true,
                        behaviour: 'drag',
                        range: {'min': 0, 'max': 1},
    ...
    class Panel extends UI.PropertyContainer {
        ...
        constructor(title, settings) {
            ...
            exports.app.panels[this.panel_id] = this;

            add_class(this.elem, "drawer");
            this.elem.dataset.id = this.panel_id;
            var header_container_elem = core_js.$(`<div class="header"><div class="inner"></div><div class="collapse-arrow"><i class="fas fa-chevron-down"></i></div></div>`)[0];
            this.body_elem = core_js.$(`<div class="body"></div>`)[0];
            ...
            var title_elem = core_js.$(`<span></span>`)[0];
            ...
            header_container_elem.addEventListener("click", (e)=>{
                ...
                if (exports.app.settings.get(setting_id) === undefined) exports.app.settings.set(setting_id, false);
                else exports.app.settings.toggle(setting_id);
            });
        }
    ...
    class StreamSettings extends Panel {
        constructor() {
            super(
                "Stream Settings",
                {
                    data: ()=>exports.app.$._session.stream_settings,
            ...
            this.properties_ui = new UI.Row({
                ...
                gap: 5,
                "align":"end",
                "hidden": ()=>exports.app.$._session._is_running || exports.app.$._session.type !== "InternalSession"
            });
            this.info_ui = new UI({
                "class":"stream-info",
                "hidden": ()=>!exports.app.$._session._is_running
            });
            ...
            this.toggle_streaming_button = new UI.Button(null, {
                id: "toggle-streaming",
                content: ()=>{
                    var state = exports.app.$._session.stream.state;
                ...
                disabled: ()=>!exports.app.$._session._is_running && !this.valid_visible,
                click: (e)=>{
                    if (exports.app.$._session._is_running) {
                        exports.app.request({
                        ...
                        if (Object.values(exports.app.$._streams).filter(s=>s._is_running).length == 0 || confirm(msg)) {
                            exports.app.request({
            ...
            this.schedule_stream_button = new UI.Button("Schedule", {
                id: "schedule-stream",
                click: (e)=>{
                    exports.app.schedule_stream_menu.show();
                },
                disabled:()=>exports.app.$._session._is_running,
                hidden:()=>exports.app.$._session._is_running
            });
            this.handover_button = new UI.Button("Handover", {
                ...
                hidden:()=>!exports.app.$._session._is_running || exports.app.$._session.type != "InternalSession" || exports.app.$._stream.test
            });
            this.config_button = new UI.Button(`<i class="fas fa-cog"></i>`, {
                ...
                hidden:()=>!exports.app.$._session._is_running || exports.app.$._stream.test
            });
            ...
            function get_default() { return core_js.utils.try(()=>exports.app.$.properties.stream_settings.props[this.name].default); }
            function get_options() { return core_js.utils.try(()=>exports.app.$.properties.stream_settings.props[this.name].options, []); }

            this.stream_method = new UI.Property("method", "Stream Method", `<select></select>`, {
                "options":function() {
                    var options = get_options.apply(this);
                    if (!exports.app.dev_mode) options = options.filter(o=>!o[1].match(/\[dev\]/i));
                    return options;
                },
                // "hidden": !app.dev_mode,
                "default": get_default,
            ...
            this.stream_title = new UI.Property("title", "Stream Title", `<input type="text">`, {
                "default": "",
                "placeholder": ()=>/* app.$.session.default_stream_title || */ exports.app.$._session.name,
                "hidden": ()=>this.stream_method.value != "rtmp",
            });
            ...
            this.test_button = new UI.Button("Test", {
                "hidden": ()=>this.stream_method.value != "rtmp",
                "disabled": ()=>!exports.app.$.processes["media-server"],
                "click": ()=> {
                    exports.app.request({
                        call: ["session", "start_stream"],
                        arguments: [{ "test": true }],
            ...
            this.on("property-change", (e)=>{
                ...
                exports.app.$._push([`sessions/${exports.app.$._session.id}/stream_settings/${e.name}`, e._value]);
            });

            this.on("post_update", ()=>{
                var session = exports.app.$._session || EMPTY_OBJECT;
                ...
                if (exports.app.$._session.type === "InternalSession") {
                    ...
                    var nms_session = exports.app.$._session._get_connected_nms_session_with_appname("livestream");
                    if (nms_session) {
                ...
                if (stream["method"] === "rtmp") {
                    if (!stream["test"]) {
                        stream_info["Stream Target(s)"] = stream["targets"].map((t,i)=>`${t} <span style="color:${exports.app.$.targets[t]?"#00f":"f00"}">[${exports.app.$.targets[t]?"OK":"NOT EXIST"}]</span>`).join(", ");
                        stream_info["Stream Title"] = stream.title;
                    }
                }
                stream_info["Run Time"] = core_js.utils.ms_to_timespan_str(exports.app.server_now - session.stream.start_time);
    ...
    class MediaPlayerPanel extends Panel {

        get video_buffer_length() {
            return core_js.utils.try(()=>(this.flv_player._mediaElement.buffered.end(0)-this.flv_player.currentTime));
        }
        ...
        constructor() {
            ...
            this.test_stream_popout_button.addEventListener("click", async (e)=>{
                var id = exports.app.$._session.id;
                var w = windows["test-"+id];
                if (w && !w.closed) {
                    w.focus();
                } else {
                    var [width,height] = exports.app.$._session.stream["resolution"].split("x").map(i=>parseInt(i));
                    ...
                    w.onload=()=>{
                        w.document.head.append(core_js.$(`<title>Test Stream ${id}</title>`)[0]);
                        /* await */ core_js.dom_utils.clone_document_head(exports.app.root_elem, w.document.head);
...
video {
    ...
    height: 100% !important;
            ...
            this.status_elem = core_js.$(`<div class="player-status"><div class="currently-playing"><span class="prefix"></span><span class="path"></span></div></div>`)[0];
            this.additional_file_info_elem = core_js.$(`<div>-</div>`)[0];
            ...
            this.seek_controls_elem = core_js.$(`<div class="seek-controls"></div>`)[0];
            ...
            this.player_controls_elem.append(
                this.prev_button = new UI.Button(`<i class="fas fa-step-backward"></i>`, {
                    ...
                    click: (e)=>{
                        exports.app.playlist_play(exports.app.$._session._current_playing_item._previous);
                    },
                    disabled:()=>!exports.app.$._session._current_playing_item._previous
                }),
                this.backward_button = new UI.Button(`<i class="fas fa-backward"></i>`, {
                    title:"-30 Seconds",
                    click: (e)=>{
                        exports.app.seek(-30,true);
                    },
                    disabled:()=>!exports.app.media.seekable || exports.app.media.time <= 0,
                }),
                this.toggle_play_pause_button = new UI.Button(null, {
                    title:"Play/Pause",
                    content: ()=>exports.app.$._stream.mpv.props.pause ? `<i class="fas fa-play"></i>` : `<i class="fas fa-pause"></i>`,
                    click: (e)=>{
                        var new_pause = !exports.app.$._stream.mpv.props.pause;
                        exports.app.request({
                            call: ["session", "mpv", "set_property"],
                            arguments: ["pause", new_pause]
                        });
                        exports.app.$._push([`streams/${exports.app.$._stream.id}/mpv/props/pause`, new_pause]);
                    },
                    disabled:()=>!exports.app.$._session._is_running,
                }),
                ...
                this.forward_button = new UI.Button(`<i class="fas fa-forward"></i>`, {
                    ...
                    disabled:()=>!exports.app.media.seekable || exports.app.media.time_left <= 0,
                }),
                this.next_button = new UI.Button(`<i class="fas fa-step-forward"></i>`, {
                    title:"Next Playlist Item",
                    click: (e)=>{
                        exports.app.playlist_play(exports.app.$._session._current_playing_item._next);
                    },
                    disabled:()=>!exports.app.$._session._current_playing_item._next
                }),
                this.prev_chapter_button = new UI.Button(`<i class="fas fa-fast-backward"></i>`, {
                    title:"Previous Chapter",
                    click: (e)=>{
                        exports.app.seek_chapter(-1,true);
                    },
                    disabled: ()=>!exports.app.settings.get("show_chapters") || exports.app.media.chapters.length == 0 || exports.app.media.time <= exports.app.media.chapters[0].start,
                ...
                this.next_chapter_button = new UI.Button(`<i class="fas fa-fast-forward"></i>`, {
                    ...
                    disabled: ()=>!exports.app.settings.get("show_chapters") || exports.app.media.chapters.length == 0 || exports.app.media.time >= exports.app.media.chapters[exports.app.media.chapters.length-1].start,
                    // hidden: ()=>!app.settings.get("show_chapters") || this.chapters.length == 0
                }),
                this.reload_button = new UI.Button(`<i class="fas fa-sync"></i>`, {
                    title:"Reload",
                    click: (e)=>{
                        exports.app.media_player.seek.seek();
                        exports.app.request({
                            call: ["session", "reload"]
                        });
                    },
                ...
                this.set_time_button = new UI.Button(`<i class="far fa-clock"></i>`, {
                    title:"Precise Seek",
                    click: (e)=>{
                        exports.app.set_time_pos_menu.show();
                    },
                    disabled:()=>!exports.app.media.seekable,
                })
            );
            ...
            this.volume.on("change", (e)=>{
                if (e.trigger) {
                    ...
                    exports.app.$._push([`sessions/${exports.app.$._session.id}/volume_target`, +e._value]);
                }
            });
            ...
            this.vol_down_button = new UI.Button(`<i class="fas fa-volume-down"></i>`, {
                ...
                title:"Volume - 5%",
                click: (e)=>{
                    this.volume.set_values(core_js.utils.ceil_to_factor(this.volume.value-VOLUME_STEP,VOLUME_STEP), {trigger:true});
                }
            });
            ...
            this.vol_up_button = new UI.Button(`<i class="fas fa-volume-up"></i>`, {
                ...
                title:"Volume + 5%",
                click: (e)=>{
                    this.volume.set_values(core_js.utils.floor_to_factor(this.volume.value+VOLUME_STEP,VOLUME_STEP), {trigger:true});
                }
            });
            ...
            this.vol_speed.on("change",(e)=>{
                if (e.trigger) {
                    ...
                    exports.app.$._push([`sessions/${exports.app.$._session.id}/volume_speed`, e._value]);
                }
            });
            ...
            this.stats_elem = core_js.$(`<div class="stats">`)[0];
            ...
            var detect_wrap = core_js.dom_utils.debounce_next_frame(()=>{
                core_js.dom_utils.detect_wrapped_elements(this.player_inline_elem);
            });
            ...
            this.on("update", ()=>{
                var started = exports.app.$._session._is_running;
                set_inner_html(this.status_prefix_elem, `${exports.app.media.status}: `);
                exports.app.build_playlist_breadcrumbs(this.status_path_elem, exports.app.media.item, true, true);
                
                var stats_html = Object.entries(exports.app.media.stats).map(([k,v])=>`<span>${k}: ${v}</span>`).join(" | ");
                set_inner_html(this.stats_elem, stats_html);
                toggle_class(this.stats_elem, "d-none", !started);
                
                if (exports.app.media.curr_chapters.length) {
                    this.additional_file_info_elem.style.display = "";
                    let html = `Chapter(s): `+exports.app.media.curr_chapters.map(c=>exports.app.chapter_to_string(c)).join(" | ");
        ...
        async refresh_player(force) {
            var test_video_url = `${window.location.protocol==="https:"?"wss:":"ws:"}//${window.location.host}/media-server/test/${exports.app.$._session.id}.flv`;
            var show = !!(exports.app.$._session._is_running && exports.app.$._stream.test && test_video_url);
            var is_popped_out = !!windows["test-"+exports.app.$._session.id];
            var is_playable = !!(show && exports.app.$._session._get_connected_nms_session_with_appname("test"));
            ...
            if (is_playable) {
                ...
                this.flv_player = core_js.flvjs.createPlayer({
                    type: "flv",
                    url: test_video_url,
                    hasAudio: true,
                    hasVideo: true,
                ...
                this.flv_player.on(core_js.flvjs.Events.MEDIA_INFO, (s)=>{
                    this.flv_media_info = s;
                });
                var initialized = false;
                this.flv_player.on(core_js.flvjs.Events.STATISTICS_INFO, (s)=>{
    ...
    class MediaSettingsPanel extends Panel {
        toggle_mode(v) {
            this._mode = v;
        ...
        constructor() {
            super("Media Settings", {
                // nullify_defaults: true,
                data: ()=>(this._mode === MediaSettingsMode.current) ? exports.app.$._session.stream.mpv.props : exports.app.$._session.player_default_override,
                disabled: ()=>this._mode === MediaSettingsMode.current && !exports.app.$._session._is_running,
            });

            this.header_elem.append(core_js.$(`<div class="buttons border-group">
    ...
    class LogViewerPanel extends Panel {
        constructor(name) {
            super(name);
            add_class(this.body_elem, "no-padding");

            this.logs_wrapper = core_js.$(`<div class="logs-wrapper"></div>`)[0];
            this.logs_container = core_js.$(`<div class="logs"></div>`)[0];
            ...
            this._logger_settings = core_js.utils.deep_copy(this._default_logger_settings);
            this.i = 0;
        
            this.storage_name = `${this.panel_id}-log-viewer-settings`;
            add_class(this.logs_container, "thin-scrollbar");
            core_js.$(this.logs_wrapper).resizable({handles:"s"});
            
            var create_toggle_button = (text)=>{
                var elem = core_js.$(`<button class="toggle-button">${text}</button>`)[0];
                return elem;
            };

            this.bar = core_js.$(`<div class="logs-bar"></div>`)[0];
            ...
            this.bar.append(core_js.$(`<div class="sep"></div>`)[0]);
            this.levels = core_js.$("<div></div>")[0];
            this.level_buttons = {};
            ...
            this.bar_wrapper = core_js.$(`<div class="logs-bar-wrapper"></div>`)[0];
        ...
        append_logs(logs) {
            var scroll_bottom = core_js.dom_utils.scroll_y_percent(this.logs_container) == 1;
            this._new_log_elems = [];
            for (var log of Object.values(logs)) {
                ...
                if (this.last_log_elem && this.last_log_elem._log_id === log_id) {
                    ...
                    log_elem = core_js.$(`<p><span class="date"></span><span class="time"></span><span class="level"></span><span class="number"></span><span class="message"></span></p>`)[0];
                    this.i++;
            ...
            if (scroll_bottom) core_js.dom_utils.scroll_y_percent(this.logs_container, 1);
            this.update_next_frame();
        }

        save() {
            exports.app.settings.set(this.storage_name, this._logger_settings);
            this.update_next_frame();
        }

        load() {
            this._logger_settings = core_js.utils.deep_copy(Object.assign({}, this._default_logger_settings, exports.app.settings.get(this.storage_name)));
        ...
        empty() {
            ...
            core_js.utils.clear(this._logs);
    ...
    class EncoderPanel extends Panel {
        constructor() {
            super("Encoder");

            var button_group = core_js.$(`<div class="buttons border-group">`)[0];
            button_group.append(core_js.$(`<button class="show_encoder_info" title="Toggle Encoder Info"><i class="fas fa-info-circle"></i></button>`)[0]);
            button_group.append(core_js.$(`<button class="pause_encoder" title="Toggle Pause"><i class="fas fa-pause"></i></button>`)[0]);
            ...
            add_class(this.body_elem, "no-padding");
            var chart_wrapper =
            core_js.$(`<div class="chart-wrapper">
            ...
            var ms_to_timespan = (value)=>{
                value = +value;
                return core_js.utils.ms_to_timespan_str(value, "hh:mm:ss")
            };
            
            this.chart = new core_js.Chart(this.chart_elem, {
                type: "line",
                data: {},
                options: {
                    animation: false,
                    ...
                    plugins: {
                        ...
                        legend: {
                            labels: {
                                boxWidth: core_js.Chart.defaults.font.size
                            }
                        }
                    },
                },
        ...
        #update_chart() {
            if (exports.app.settings.get("pause_encoder") && this.chart.data.datasets.length) return;

            var raw_data = exports.app.$._session.stream.speed_history;
            ...
            for (var [g,values] of Object.entries(raw_data)) {
                ...
                var avg = core_js.utils.average(y_values);
            ...
            var table = core_js.dom_utils.build_table(info_rows);
    ...
    const PLAYLIST_ZOOM_MIN = 0.01;
    const PLAYLIST_ZOOM_MAX = 100;
    const PLAYLIST_ZOOM_BASE = 1.3333;
    class PlaylistPanel extends Panel {
        ...
        set_tracks(num_tracks, is_2_track) {
            ...
            this._tracks = tracks;
            core_js.dom_utils.empty(this.tracks_elem);
            core_js.dom_utils.empty(this.headers_elem);
            core_js.dom_utils.empty(this.highlights_elem);
            ...
            this.sortables = tracks.map((t,i)=>{
                ...
                var playlist_elem = core_js.$(`<ul class="playlist"></ul>`)[0];
                this.tracks_elem.append(playlist_elem);

                var playlist_header = core_js.$(`<div>${t.header}</div>`)[0];
                ...
                var playlist_highlight = core_js.$(`<div></div>`)[0];
        ...
        scroll_into_view(elem, opts) {
            ...
            if (this.orientation === Orientation.VERTICAL) {
                ...
                var bottom = exports.app.main_elem.offsetHeight-40;
                if (r.bottom > bottom) {
                    exports.app.main_elem.scrollBy(0, r.bottom-bottom);
                }
            }
        }
        ...
        /** @return {PlaylistItem} */
        get current() { return this._current || exports.app.$._session.playlist["0"] || NULL_PLAYLIST_ITEM; }
        ...
        <div class="playlist-controls">
            ...
            this.timeline_container_elem.addEventListener("dblclick", (e) => {
                ...
                if (!item_el) return;
                var item = exports.app.$._session.playlist[item_el.dataset.id];
                exports.app.playlist_play(item);
                // elem.ondblclick = ()=>app.playlist_play(item);
            });

            this.timeline_container_elem.addEventListener("contextmenu", (e) => {
                if (e.target.contentEditable == "true") return;
                var sortable = core_js.Sortable.utils.get(e.target);
                if (sortable) {
                    e.preventDefault();
                    e.stopPropagation();
            ...
            this.timeline_mode_select.addEventListener("change", (e)=>{
                exports.app.settings.set("playlist_mode", +this.timeline_mode_select.value);
                this.scroll_to_playhead();
            });
            ...
            this.pl_add_file_remote_button.addEventListener("click", async (e)=>{
                ...
                if (paths) exports.app.playlist_add(paths);
            });
            this.pl_add_file_local_button.addEventListener("click", async (e)=>{
                var files = await core_js.dom_utils.open_file_dialog({multiple:true}); // directories:true
                exports.app.playlist_add(files);
            });

            this.pl_add_url_button.addEventListener("click", async (e)=>{
                var urls_str = await new Promise((resolve)=>{
                    exports.app.playlist_add_url_menu.show(resolve);
                });

                if (urls_str) {
                    var urls = urls_str.split(/\n+/).map(s=>s.trim()).filter(s=>s);
                    exports.app.playlist_add(urls);
                }
            });

            core_js.dom_utils.tippy(this.pl_add_other_button, {
                appendTo: exports.app.elem,
                allowHTML: true,
                trigger: "click",
                // hideOnClick: "toggle",
            ...
            this.commands = {
                play: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.playlist_play(items[0]);
                    },
                    shortcut: "P",
                }),
                info: new PlaylistCommand({
                    ...
                    visible: (items)=>items.length>0,
                    click: (items)=>{
                        exports.app.playlist_info_menu.show(items);
                    },
                    shortcut: "I",
                }),
                modify: new PlaylistCommand({
                    ...
                    visible: (items)=>items.length>0,
                    click: (items)=>{
                        exports.app.playlist_modify_menu.show(items);
                    },
                    shortcut: "M",
                }),
                delete_item: new PlaylistCommand({
                    ...
                    visible: (items)=>items.length>0,
                    click: (items)=>{
                        exports.app.playlist_remove(items);
                    },
                    shortcut: "Delete",
                }),
                rescan: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.playlist_rescan(items);
                    },
                    shortcut: "R",
                }),
                reveal: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.navigate_to(items.map(i=>i.filename));
                    },
                    shortcut: "Ctrl+F",
                }),
                download: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.playlist_download(items);
                    },
                }),
                cancel_download: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.playlist_cancel_download(items);
                    }
                }),
                cancel_upload: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.playlist_cancel_upload(items);
                    }
                }),
                ...
                add_to_playlist: new PlaylistCommand({
                    ...
                    visible: (items)=>items.length>0,
                    click: (items)=>{
                        exports.app.playlist_group(items);
                    },
                    shortcut: "Ctrl+G",
                }),
                breakdown_playlist: new PlaylistCommand({
                    ...
                    // disabled: (items)=>!items.every(i=>i.is_playlist),
                    click: (items)=>{
                        exports.app.playlist_breakdown(items);
                    },
                    shortcut: "Ctrl+U",
                }),
                split: new PlaylistCommand({
                    ...
                    click: (items)=>{
                        exports.app.split_menu.show(items);
                    }
                }),
                slice_at_timeline_cursor: new PlaylistCommand({
                    ...
                    disabled: (items)=>!items.every(i=>i._is_splittable),
                    click: (items)=>{
                        exports.app.playlist_split(items, [this.cursor_position], false, true);
                    },
                    shortcut: "S",
                ...
                add_intertitle: new PlaylistCommand({
                    ...
                    icon: `<i class="fas fa-plus"></i>`,
                    click: async()=>{
                        await exports.app.playlist_add({
                ...
                unload_current: new PlaylistCommand({
                    ...
                    disabled: ()=>!exports.app.$._session._current_playing_item !== NULL_PLAYLIST_ITEM,
                    click: ()=>exports.app.playlist_play(NULL_PLAYLIST_ITEM),
                }),
                rescan_all: new PlaylistCommand({
                    name: ()=> "Rescan All",
                    icon: `<i class="fas fa-sync-alt"></i>`,
                    // click: ()=>app.playlist_rescan_all(),
                    click: ()=>exports.app.playlist_rescan(this.current._children),
                }),
                save_playlist: new PlaylistCommand({
                    name: ()=>"Save Playlist...",
                    icon: `<i class="fas fa-save"></i>`,
                    click: ()=>exports.app.save_playlist_menu.show(),
                }),
                generate_schedule: new PlaylistCommand({
                    name: ()=>"Generate Schedule...",
                    icon: `<i class="fas fa-calendar-alt"></i>`,
                    click: ()=>exports.app.schedule_generator_menu.show()
                })
            };
            ...
            for (let color_key in item_colors) {
                ...
                this.commands[`color_${color_key}`] = new PlaylistCommand({
                    click: (items)=>{
                        ...
                        exports.app.$._push([`sessions/${exports.app.$._session.id}/playlist/${i.id}/props/color`, v]);
                    },
                    render:(items, elem)=>{
                        add_class(elem, "color");
                        var colors = new Set(items.map(i=>i.props.color||"none"));
                        var inner = colors.has(color_key) ? ((colors.size == 1) ? `✓` : "-") : "";
                        var el = core_js.$(`<div class="color" style="background: ${color}; outline: 1px solid #ddd; text-align: center;">${inner}</div>`)[0];
                        elem.append(el);
                    },
                    title: core_js.utils.capitalize(color_key),
                });
            }
...

            {
                ...
                let mc = new core_js.Hammer.Manager(this.timeline_container_elem, {touchAction: 'none', cssProps: {userSelect:"auto"}});
                let pinch = new core_js.Hammer.Pinch({enable:true});
                mc.add([pinch]);
                let x_percent;
                let init_zoom;
                mc.on("pinchstart", (e)=>{
            ...
            window.addEventListener("keydown", this.on_keydown = (e)=>{
                if (core_js.dom_utils.has_focus(this.timeline_container_elem)) {
            ...
            exports.app.main_elem.addEventListener("scroll", on_scroll=()=>this.#update_position());
            ...
            this.on("update", ()=>{
                ...
                var self_and_parents = [exports.app.$._session._current_playing_item, ...exports.app.$._session._current_playing_item._parents];
                var a_index = self_and_parents.indexOf(current);
                var timeline_time = core_js.utils.sum(self_and_parents.slice(0, a_index).map(item=>core_js.utils.try(()=>item._userdata.timeline_start)||0)) + Math.min(exports.app.$._session._current_time, exports.app.$._session._current_playing_item._userdata.timeline_duration);
                var time = core_js.utils.sum(self_and_parents.slice(0, a_index).map(item=>core_js.utils.try(()=>item._userdata.start)||0)) + Math.min(exports.app.$._session._current_time, exports.app.$._session._current_playing_item._userdata.duration);
        
                this.time = timeline_time;
                this.duration = timeline_duration;
                this.clipping = current_ud.clipping;
                if (this.clipping) {
                    this.clip_time = core_js.utils.loop(this.time + this.clipping.offset + this.clipping.start, this.clipping.start, this.clipping.end);
                } else {
                ...
                set_inner_html(this.playlist_time_total_elem, `(${core_js.utils.seconds_to_timespan_str(duration)})`);
                set_inner_html(this.playlist_time_left_elem, `[-${core_js.utils.seconds_to_timespan_str(duration-time)}]`);
        
                this.playlist_time_left_elem.style.display = current === exports.app.$._session.playlist["0"] ? "" : "none";
                
                exports.app.build_playlist_breadcrumbs(this.playlist_path_text, current, true);
            ...
            this.on("destroy", ()=>{
                ...
                exports.app.main_elem.removeEventListener("scroll", on_scroll);
                this.sortables.forEach(s=>s.destroy());
                this.sortables = [];
            });
        ...
        async #rebuild_items() {
            ...
            let is_running = exports.app.$._session._is_running;
            ...
            /** @type {PlaylistItem} */
            var current_item = exports.app.$._session._current_playing_item;
            ...
            this.sortables.forEach((sortable,i)=>{
                ...
                core_js.dom_utils.rebuild(sortable.el, items, {
                    add: (item, elem, index)=>{
                        if (!elem) {
                            elem = core_js.$(`<li class="item"><div><div class="clips"></div><div class="front"><span class="play-icons"></span><span class="icons"></span><span class="filename"></span><span class="extra"></span><span class="badges"></span><div class="duration"></div></div></div></li>`)[0];
                        }
                        ...
                        if (is_upload_dummy) {
                            ...
                            } else if (!core_js.utils.is_empty(media_info) && !media_info.streams && media_info.protocol !== "livestreamer:" && !item._is_playlist && !ud.is_processing) {
                        ...
                        if (item.props.color) {
                            ...
                            outline_color = new core_js.utils.Color(item_colors[item.props.color]).rgb_mix("#000",0.3).to_rgb_hex();
                        }
                        if (is_playlist) {
                        ...
                        if (media_info.downloadable && item.filename.match(/^https?:/)) {
                            let icon = core_js.$(`<i class="fas fa-globe"></i>`)[0]; //  style="color:cornflowerblue"
                        ...
                        if (!ud.download) {
                            if (media_info.streams) {
                                var default_video = core_js.utils.sort(media_info.streams.filter(s=>s.type === "video"),
                                    (s)=>s.albumart,
                                    (s)=>[s.default | s.forced * 2, "DESCENDING"]
                                )[0];
                                var default_audio = core_js.utils.sort(media_info.streams.filter(s=>s.type === "audio"),
                                    (s)=>[s.default | s.forced * 2, "DESCENDING"]
                                )[0];
                        ...
                        }
                        {
                            ...
                            if (d) {
                                let bar = extra_elem.querySelector(`.progress`) || core_js.$(`<div class="progress"><span class="percent"></span><span class="speed"></span></div>`)[0];
                                let icon = extra_elem.querySelector(`.fas`) || core_js.$(`<i class="fas"></i>`)[0];
                                
                                let p = d.total ? ( d.bytes / d.total) : 0;
                                set_attribute(bar, "title", `${core_js.utils.capitalize(t)}ing [${core_js.utils.format_bytes(d.bytes || 0)} / ${core_js.utils.format_bytes(d.total || 0)}]`);
                                ...
                                set_inner_html(bar.querySelector(".speed"), `${core_js.utils.format_bytes(d.speed || 0)}ps`);
                        ...
                        // }
                        if (!duration_str) {
                            if (ud.duration || ud.media_duration) duration_str = core_js.utils.seconds_to_timespan_str(ud.duration || ud.children_duration, "h?:mm:ss");
                        }

                        set_inner_html(duration_elem, duration_str || "  -  ");
                        
                        if (problems.length) {
                            var problem_groups = core_js.utils.group_by(problems, p=>p.level);
                            var err_icon_html;
                            ...
                            if (err_icon_html) {
                                let icon = core_js.$(err_icon_html)[0];
                    ...
                    remove:(elem)=>{
                        ...
                        var sortable = core_js.Sortable.utils.get(elem);
        ...
        back(){
            ...
            if (!parent) return;
            if (exports.app.$._session._is_running && current == exports.app.$._session._current_playing_item && current._calculate_contents_hash() != current.__private.hash_on_open) {
                exports.app.prompt_for_reload_of_current_item();
            }
        ...
        move_selection_back() {
            var elems = this.get_selection();
            var first_index = Math.max(0,core_js.dom_utils.get_index(elems[0])-1);
            elems.forEach((e,i)=>core_js.dom_utils.insert_at(e.parentElement, e, first_index+i));
            this.scroll_into_view(elems[0]);
            this.sync();
        }
        move_selection_forward() {
            var elems = this.get_selection();
            var last_index = Math.min(elems[0].parentElement.childElementCount, core_js.dom_utils.get_index(elems[elems.length-1])+2);
            elems.forEach((e,i)=>core_js.dom_utils.insert_at(e.parentElement, e, last_index));
        ...
        sync() {
            ...
            exports.app.playlist_update(playlist_changes);
        }
        ...
        async clipboard_paste() {
            ...
            if (clipboard.cutting) {
                this.clipboard = null;
                exports.app.playlist_move(clipboard.items);
            } else {
                exports.app.playlist_add(clipboard.items);
            }
        }
        ...
        rename(item) {
            ...
            filename.addEventListener("blur", blur_listener = ()=>{
                ...
                if (old_name != new_name) {
                    ...
                    exports.app.$._push([`sessions/${exports.app.$._session.id}/playlist/${item.id}/props/label`,new_name]);
                }
        ...
        set_selection(items) {
            ...
            elems.forEach((elem,i)=>{
                core_js.Sortable.utils.get(elem).click(elem, false, i!=0);
            });
        }
        ...
        get_datas() {
            return this.get_elements().map(e=>exports.app.$._session.playlist[e.dataset.id]).filter(i=>i);
        }
        get_selection_datas() {
            return this.get_selection().map(e=>exports.app.$._session.playlist[e.dataset.id]).filter(i=>i);
        }
        get_selection_indices() {
            return this.get_selection().map(e=>core_js.dom_utils.get_index(e));
        }
        ...
        /** @param {PlaylistItem} item */
        open(item, selection) {
            if (!item) item = exports.app.$._session.playlist["0"];
        ...
        #update_info() {
            ...
            if (selected_items.length) {
                var duration = core_js.utils.sum(selected_items.map(i=>i._userdata.duration));
                info["Duration"] = `<i class="far fa-clock"></i> (${core_js.utils.seconds_to_timespan_str(duration, "h?:mm:ss")})`;
            }
            if (this.clipboard) {
        ...
        #update_position() {
            ...
            var get_style = ()=>{
                if (!exports.app.settings.get("playlist_sticky")) return;
                ...
                height = core_js.utils.clamp(height, min_height, max_height);
        ...
        set_timeline_zoom(v){
            this.zoom = core_js.utils.clamp(v, PLAYLIST_ZOOM_MIN, PLAYLIST_ZOOM_MAX);
        ...
        inc_timeline_zoom(v=0, e) {
            this.set_timeline_view(Math.pow(PLAYLIST_ZOOM_BASE, core_js.utils.log(this.zoom, PLAYLIST_ZOOM_BASE) + v), null, e);
        }
        set_timeline_view(zoom, time, e=null) {
            ...
            if (e instanceof MouseEvent) {
                ...
                ox = core_js.utils.clamp(get_rect_pt_percent(rect, pt).x);
    ...
    class Area extends UI.Column {
        constructor(elem, settings) {
            super(elem, settings);
            add_class(this.elem, "area");
            add_class(this.elem, `area-${exports.app.areas.length+1}`);
            exports.app.areas.push(this);
        }
    }

    class App extends core_js.utils.EventEmitter {
        #initialized = false;
        get server_now() { return Date.now() + this.$.server_time_diff; }
        get playlist_item_props_class() { return core_js.utils.try(()=>this.$.properties.playlist.enumerable_props.props.props); }
        ...
        constructor() {
            super();
            exports.app = this;
            core_js.jQuery(async ()=>{
                this.loader = new Loader();
                this.loader.update({visible:true, text:"Initializing..."});

                this.settings = new core_js.dom_utils.LocalStorageBucket("livestreamer-1.0", {
                    "playlist_mode": 0,
                ...
                this.passwords = new core_js.dom_utils.LocalStorageBucket("livestreamer-passwords");

                var messenger = new core_js.dom_utils.WindowCommunicator();
                var key;
                if (window.self == window.top) {
                    key = core_js.dom_utils.Cookie.get("ls_key") || new URLSearchParams(window.location.search).get("ls_key");
                } else {
                    key = await messenger.request(window.parent, "key");
                }
                if (key) core_js.dom_utils.Cookie.set("ls_key", key, { expires: 365 });
                messenger.destroy();
                ...
                this.ws = new core_js.dom_utils.WebSocket(ws_url+"?"+ws_params.toString());
                this.ws.on("open", ()=>{
                    this.$ = new Remote();
        ...
        init() {
            ...
            this.elem.addEventListener("click",(e)=>{
                ...
                if (e.target.matches("a")) {
                    var url = core_js.dom_utils.get_anchor_url(e.target);
            ...
            core_js.dom_utils.tippy.setDefaultProps({
                distance: 0,
            ...
            core_js.$(this.new_session_button).on("click", (e)=>{
                this.request({
                    call: ["new_session"]
                }, {
                    block: true,
...

            {
                ...
                let button = core_js.$(`<button class="button" title="Sign in"><i class="fas fa-key"></i></button>`)[0];
            ...
            for (let area of this.areas) {
                new core_js.Sortable(area.elem, {
                    group: "layout",
                    fallbackTolerance: 3, // So that we can select items on mobile
                    animation: 150,
                    ...
                    filter: (e)=>{
                        if (e.target.closest(".drawer>.header")) {
                            if (core_js.dom_utils.has_touch_screen() || e.target.closest("button,input,select")) return true;
                        }
                    },
            ...
            fd.on("drop", async (files)=>{
                if (files.length) exports.app.playlist_add(files);
            });
            ...
            var row = new UI.Row({
                gap: 0,
                visible: ()=>exports.app.$.processes["file-manager"] || IS_ELECTRON
            });
            ...
            if (!IS_ELECTRON) {
                row.append(
                    new UI.Button(`<i class="fas fa-folder-tree"></i>`, {
                        click: ()=>{
                            exports.app.file_system_info_menu.show();
                        },
                        title: ()=>exports.app.file_system_info_menu.modal_title
                    })
                );
            }
            row.append(
            ...
            this.footer_buttons.append(
                ...
                new UI.Button(`Change Log`, {
                    ...
                    /** @this {UI} */
                    update: function() {
                        var diff = (exports.app.settings.get("last_change_log") != exports.app.$.change_log.mtime);
        ...
        update(changes) {
            ...
            var has_access = is_null_session || access_control.self_has_access(exports.app.passwords.get(this.$._session.id)) || this.$._client.is_admin;
            var requires_password = access_control.self_requires_password;
            var is_external_session = this.$._session.type === "ExternalSession";
            this.$._session._is_running;
...

            {
                ...
                if (changes.downloads) {
                    core_js.utils.set_add(ids, Object.keys(changes.downloads));
                }
                if (changes.uploads) {
                    core_js.utils.set_add(ids, Object.keys(changes.uploads));
                }
                if (changes.uploads) {
                ...
                if (changes.media_info) {
                    core_js.utils.set_add(filenames, Object.keys(changes.media_info));
                }
                if (changes.nms_sessions !== undefined) {
                    core_js.utils.set_add(ids, this.playlist.current._children.filter(i=>i.filename==="livestreamer://rtmp").map(i=>i.id));
                }
                if (session_changes) {
                    ...
                    if (session_changes.playlist) {
                        core_js.utils.set_add(ids, Object.keys(session_changes.playlist));
                    }
                    ...
                    if (session_changes.playlist_id !== undefined) {
                        core_js.utils.set_add(ids, [this.playlist.current.id, this.$._session.playlist_id]);
                    }
                }
                core_js.utils.set_add(ids, [...filenames].map(k=>this.$._session._get_items_with_media(k)).flat().map(i=>i.id));
            ...
            core_js.utils.clear(this.$._session._deleted_playlist_items);
            
            if (!core_js.utils.is_empty(changes)) {
        ...
        media = new class {
            ...
            loaded;
            update() {
                this.item = exports.app.$._session._current_playing_item;
                var started = exports.app.$._session._is_running;
                var loaded = !started || !!exports.app.$._stream.mpv.loaded;
                var seeking = started && !!exports.app.$._stream.mpv.seeking;
                var special_seeking = started && !!exports.app.$._stream.mpv.is_special && !!exports.app.$._stream.mpv.seeking;
                var buffering = !!(seeking || !loaded || exports.app.$._stream.mpv.props["paused-for-cache"]);

                if (special_seeking) return;
                this.time = exports.app.$._session._current_time;
                this.duration = exports.app.$._session._current_duration;
                this.chapters = exports.app.$._session._current_chapters;
                this.seekable = this.duration != 0 && (!started || !!exports.app.$._stream.mpv.seekable) && this.item.filename !== "livestreamer://empty";
                this.loaded = loaded;
                ...
                this.stats = {};
                this.stats["V-FPS"] = (+exports.app.$._stream.mpv.props["estimated-vf-fps"] || 0).toFixed(2);
                if (!exports.app.$._stream._is_encoding) this.stats["D-FPS"] = (+exports.app.$._stream.mpv.props["estimated-display-fps"] || 0).toFixed(2);
                this.stats["INTRP"] = exports.app.$._stream.mpv.interpolation ? "On" : "Off";
                this.stats["DEINT"] = exports.app.$._stream.mpv.deinterlace ? "On" : "Off";

                this.curr_chapters = exports.app.$._session._get_current_chapters_at_time(this.time);
            }
            get time_left() { return Math.max(0, this.duration - this.time); }
            get do_live_seek(){ return exports.app.$._stream._is_running && !exports.app.$._stream._is_encoding && !exports.app.$._stream.mpv.is_special; }
        }
        ...
        load_font(id) {
            if (!this.font_cache[id]) {
                this.font_cache[id] = exports.app.request({
                    call: ["app", "get_font"],
                    arguments: [id]
                });
            }
        ...
        playlist_split(items, splits, local_times=false) {
            if (!Array.isArray(items)) items = [items];
            if (!items.length) return;
            splits = core_js.utils.sort(splits);
            ...
            for (var item of items) {
                ...
                for (var i = 0; i <= splits.length; i++) {
                    ...
                    if (!(core_js.utils.almost_equal(segment_start, start) && core_js.utils.almost_equal(segment_end, end)) && segment_start >= start && segment_end <= end && d>0 && !core_js.utils.almost_equal(d, 0)) {
        ...
        playlist_move(items, pos=null, track_index=null) {
            ...
            var is_circular = core_js.utils.is_circular(all);
            if (is_circular) {
            ...
            for (var [session_id, group] of Object.entries(core_js.utils.group_by(items, i=>i._session.id))) {
        ...
        playlist_add(items, insert_pos=null, track_index=null) {
            ...
            if (track_index == null) track_index = this.playlist.active_track_index;
            track_index = core_js.utils.clamp(track_index, 0, 1);
            ...
            var add_file = (d,i,parent_id,track_index)=>{
                let filename = null;
                var item = {};
                var id = core_js.dom_utils.uuid4();
        ...
        async playlist_remove(items) {
            ...
            for (var item of items) {
                var ul = item._upload;
                if (ul) exports.app.upload_queue.cancel(ul.id);
            }
            for (var [session_id, group] of Object.entries(core_js.utils.group_by(items, i=>i._session.id))) {
        ...
        async playlist_update(changes, session_id) {
            if (!session_id) session_id = exports.app.$._session.id;
            changes = core_js.utils.tree_from_entries(changes);
        ...
        playlist_cancel_upload(items) {
            if (!Array.isArray(items)) items = [items];
            items.forEach(i=>exports.app.upload_queue.cancel(i.id));
            if (items.length == 0) return;
            ...
            if (root_merged) {
                ...
                for (var p of [item, ...item._get_parents(root_merged)]) {
                    ...
                    if (ud.clipping) {
                        ...
                        t = core_js.utils.loop(t - ud.clipping.offset, ud.clipping.start, ud.clipping.end) - ud.clipping.start;
                    }
                }
                item = root_merged;
                start += t;
        ...
        navigate_to(uris) {
            ...
            for (var uri of uris) {
                ...
                seen.add(uri);
                var is_file = !core_js.utils.is_uri(uri) || uri.startsWith("file:");
                if (IS_ELECTRON) {
                    if (is_file) electron.shell.showItemInFolder(core_js.utils.try_file_uri_to_path(uri.replace(/\//g, "\\")));
                    else electron.shell.openExternal(uri);
                } else {
                    if (is_file) open_file_manager({start: core_js.utils.try_file_uri_to_path(uri)});
        ...
        seek(t, relative=false) {
            ...
            exports.app.$._push([`sessions/${exports.app.$._session.id}/time`, t]);
        ...
        seek_chapter(i, relative=false) {
            ...
            i = core_js.utils.clamp(i, 0, chapters.length-1);
            c = chapters[i];
            if (c) {
        ...
        fix_insert_pos(pos, track_index) {
            ...
            if (pos == null) {
                ...
                pos = (last_active) ? core_js.dom_utils.get_index(last_active) + 1 : num_items;
            }
            pos = core_js.utils.clamp(pos, 0, num_items);
            return pos;
        }
        ...
        build_playlist_breadcrumbs(parent_elem, item, exclude_root=false, bold_current=false) {
            ...
            core_js.dom_utils.empty(parent_elem);
            add_class(parent_elem, "breadcrumbs");
            path.forEach((item,i)=>{
                var elem = core_js.$(`<a></a>`)[0];
                ...
                if (i != path.length-1) {
                    parent_elem.append(core_js.$(`<i></i>`)[0]);
                }
            });
        }
        ...
        get_handover_sessions_options(include_none=true) {
            ...
            names = core_js.utils.uniquify(names, (s,i,n)=>n>1?`${s} [${i+1}]`:s);
        ...
        request(data, opts) {
            ...
            return new Promise(async (resolve)=>{
                ...
                this.last_request = ws_promise
                    .then(d=>resolve(d))
                    .catch((e)=>{
                        if (e instanceof core_js.utils.TimeoutError) return;
                        if (this.dev_mode) {
        ...
        tick() {
            toggle_class(this.body_elem, "is-touch", core_js.dom_utils.has_touch_screen());
        }
        ...
        #rebuild_clients() {
            ...
            var items = [
                ...
                ...core_js.utils.sort(Object.values(clients_stacked), a=>a[0].username).map(c=>({username:c[0].username, type:"client", number:c.length}))
            ];
            for (var i of items) i.id = core_js.utils.md5(JSON.stringify(i));

            core_js.dom_utils.rebuild(this.users_elem, items, {
                add: (item, elem, i)=>{
                    var elem = core_js.$(`<span class="user"></span>`)[0];
                    var is_self = this.$._client.username == item.username;
                    var text = is_self ? `Me` : item.username;
                    toggle_class(elem, "is-self", is_self);
                    if (item.type === "owner") {
                        elem.append(core_js.$(`<i class="fas fa-user-tie"></i>`)[0]);
                        add_class(elem, "is-owner");
                    } else {
                        elem.append(core_js.$(`<i class="fas fa-user"></i>`)[0]);
                        if (item.number > 1) text += ` (${item.number})`;
                    }
                    elem.append(core_js.$(`<span>${text}</span>`)[0]);
                    set_attribute(elem, "title", `${item.username} (${core_js.utils.capitalize(item.type)})`);
                    return elem;
                }
            });
        }
        ...
        get sessions_ordered() {
            if (this.$.conf["main.session-order-client"]) {
                ...
                return core_js.utils.sort(Object.values(this.$.sessions), (s)=>{
                    var i = order.indexOf(s.id);
                    if (i == -1) return Number.MAX_SAFE_INTEGER;
                    return i;
                }, (s)=>s.index);
            } else {
                return core_js.utils.sort(Object.values(this.$.sessions), (s)=>s.index);
            }
        }
        ...
        async #rebuild_sessions() {
            ...
            var session_id = this.$._client.session_id;
            core_js.dom_utils.rebuild(this.sessions_tabs_elem, items, {
                add: (item, elem, i)=>{
                    if (!elem) elem = core_js.$(`<a class="session-tab"><div class="handle"><i class="fas fa-grip-lines"></i></div><span class="name"></span><span class="icons"></span></a>`)[0];
                    var access_control = new AccessControl(item.access_control);
                    var has_access = access_control.self_has_access(exports.app.passwords.get(item.id));
        ...
        async save_session() {
            ...
            var name = `${core_js.utils.sanitize_filename(this.$._session.name)}-${core_js.utils.date_to_string()}`;
            await save_local_file(`${name}.json`, JSON.stringify(data, null, "  "));
        }

        async toggle_help() {
            if (!this.help_container) {
                this.help_container = core_js.$(await fetch("./help.html").then(d=>d.text()))[0];
        ...
        chapter_to_string(c, show_time=false) {
            ...
            if (show_time) parts.push(`[${core_js.utils.seconds_to_timespan_str(c.start)}]`);
            return parts.join(" ");
        }

        filename_to_elfinder_hash(uri) {
            if (!core_js.utils.is_uri(uri)) uri = core_js.utils.path_to_file_uri(uri);
    ...
    Object.defineProperty(exports, "dom_utils", {
        enumerable: true,
        get: function () { return core_js.dom_utils; }
    });
    Object.defineProperty(exports, "utils", {
        enumerable: true,
        get: function () { return core_js.utils; }
    });
    exports.ALL_XHRS = ALL_XHRS;
    exports.AccessControl = AccessControl;
    exports.AccessControlProperty = AccessControlProperty;
    exports.AdminSettings = AdminSettings;
    exports.App = App;
    exports.Area = Area;
    exports.CHUNK_SIZE = CHUNK_SIZE;
    exports.CROP_LIMIT = CROP_LIMIT;
    exports.CancelSortPlugin = CancelSortPlugin;
    exports.ChangeLog = ChangeLog;
    exports.Client = Client;
    exports.CropPreview = CropPreview;
    exports.DataNode = DataNode;
    exports.ELFINDER_USE_FILE_PROTOCOL = ELFINDER_USE_FILE_PROTOCOL;
    exports.EMPTY_ARRAY = EMPTY_ARRAY;
    exports.EMPTY_OBJECT = EMPTY_OBJECT;
    exports.EditAccessControlMemberMenu = EditAccessControlMemberMenu;
    exports.EncoderPanel = EncoderPanel;
    exports.ExternalSessionConfigurationMenu = ExternalSessionConfigurationMenu;
    exports.FileDrop = FileDrop;
    exports.FileManagerMenu = FileManagerMenu;
    exports.FileProperty = FileProperty;
    exports.FileSystemInfoMenu = FileSystemInfoMenu;
    exports.FontSettings = FontSettings;
    exports.HandoverSessionMenu = HandoverSessionMenu;
    exports.HistorySettings = HistorySettings;
    exports.IMAGE_DURATION = IMAGE_DURATION;
    exports.IS_ELECTRON = IS_ELECTRON;
    exports.InfoSettings = InfoSettings;
    exports.JSONContainer = JSONContainer;
    exports.JSONViewer = JSONViewer;
    exports.KeyboardShortcuts = KeyboardShortcuts;
    exports.Loader = Loader;
    exports.LocalServerTargetConfigMenu = LocalServerTargetConfigMenu;
    exports.LogViewerPanel = LogViewerPanel;
    exports.MAX_CLIP_SEGMENTS = MAX_CLIP_SEGMENTS;
    exports.MIN_VIDEO_BUFFER_TIME = MIN_VIDEO_BUFFER_TIME;
    exports.MPV = MPV;
    exports.MediaPlayerPanel = MediaPlayerPanel;
    exports.MediaSeekBar = MediaSeekBar;
    exports.MediaSettingsInterface = MediaSettingsInterface;
    exports.MediaSettingsMode = MediaSettingsMode;
    exports.MediaSettingsPanel = MediaSettingsPanel;
    exports.ModalPropertyContainer = ModalPropertyContainer;
    exports.NOOP = NOOP;
    exports.NULL_CLIENT = NULL_CLIENT;
    exports.NULL_PLAYLIST_ITEM = NULL_PLAYLIST_ITEM;
    exports.NULL_SESSION = NULL_SESSION;
    exports.NULL_STREAM = NULL_STREAM;
    exports.Orientation = Orientation;
    exports.PLAYLIST_MODE = PLAYLIST_MODE;
    exports.PLAYLIST_VIEW = PLAYLIST_VIEW;
    exports.PLAYLIST_ZOOM_BASE = PLAYLIST_ZOOM_BASE;
    exports.PLAYLIST_ZOOM_MAX = PLAYLIST_ZOOM_MAX;
    exports.PLAYLIST_ZOOM_MIN = PLAYLIST_ZOOM_MIN;
    exports.Panel = Panel;
    exports.PlaylistAddURLMenu = PlaylistAddURLMenu;
    exports.PlaylistCommand = PlaylistCommand;
    exports.PlaylistItem = PlaylistItem;
    exports.PlaylistModifySettings = PlaylistModifySettings;
    exports.PlaylistPanel = PlaylistPanel;
    exports.RangeProperty = RangeProperty;
    exports.Remote = Remote;
    exports.ResponsiveSortable = ResponsiveSortable;
    exports.SavePlaylistSettings = SavePlaylistSettings;
    exports.ScheduleGenerator = ScheduleGenerator;
    exports.ScheduleStreamSettings = ScheduleStreamSettings;
    exports.SeekBar = SeekBar;
    exports.SelectableList = SelectableList;
    exports.Session = Session;
    exports.SessionConfigurationSettings = SessionConfigurationSettings;
    exports.SetTimePosSettings = SetTimePosSettings;
    exports.SetVolumeSettings = SetVolumeSettings;
    exports.SplitSettings = SplitSettings;
    exports.Stream = Stream;
    exports.StreamConfigurationMenu = StreamConfigurationMenu;
    exports.StreamKeyGeneratorSettings = StreamKeyGeneratorSettings;
    exports.StreamSettings = StreamSettings;
    exports.SystemManagerMenu = SystemManagerMenu;
    exports.Target = Target;
    exports.TargetConfigurationMenu = TargetConfigurationMenu;
    exports.TargetEditMenu = TargetEditMenu;
    exports.TargetsProperty = TargetsProperty;
    exports.TicksBar = TicksBar;
    exports.TimeLeftMode = TimeLeftMode;
    exports.UI = UI;
    exports.UPLOAD_STATUS = UPLOAD_STATUS;
    exports.UploadFileChunk = UploadFileChunk;
    exports.UploadQueue = UploadQueue;
    exports.UploadStatus = UploadStatus;
    exports.UploadsDownloadsMenu = UploadsDownloadsMenu;
    exports.UserConfigurationSettings = UserConfigurationSettings;
    exports.VOLUME_STEP = VOLUME_STEP;
    exports.WS_MIN_WAIT = WS_MIN_WAIT;
    exports.YES_OR_NO = YES_OR_NO;
    exports.ZERO_DURATION = ZERO_DURATION;
    exports.add_class = add_class;
    exports.create_background_properties = create_background_properties;
    exports.create_context_menu = create_context_menu;
    exports.create_menu = create_menu;
    exports.create_proxy = create_proxy;
    exports.cull = cull;
    exports.default_file_manager_options = default_file_manager_options;
    exports.fancybox_prompt = fancybox_prompt;
    exports.get_clip_segments = get_clip_segments;
    exports.get_or_call = get_or_call;
    exports.get_rect_pt_percent = get_rect_pt_percent;
    exports.get_scrollbar_width = get_scrollbar_width;
    exports.get_video_size = get_video_size;
    exports.graph_colors = graph_colors;
    exports.hash = hash;
    exports.ignore_logging_session_$ = ignore_logging_session_$;
    exports.item_colors = item_colors;
    exports.logs_max_length = logs_max_length;
    exports.mime_ext_map = mime_ext_map;
    exports.moving_average = moving_average;
    exports.ondrag = ondrag;
    exports.pretty_uri_basename = pretty_uri_basename;
    exports.rect_clamp_point = rect_clamp_point;
    exports.remove_class = remove_class;
    exports.remove_empty_objects_from_tree = remove_empty_objects_from_tree;
    exports.remove_style_property = remove_style_property;
    ...
    exports.set_select_options = set_select_options;
    exports.set_style_property = set_style_property;
    exports.set_text = set_text;
    exports.set_value = set_value;
    exports.time_to_minutes = time_to_minutes;
    exports.toggle_attribute = toggle_attribute;
    exports.toggle_class = toggle_class;
    exports.update_style_properties = update_style_properties;
